...
├── architecture/        ← 통합 아키텍처 문서 (55 md + INDEX.md 색인, 실무 참조, archive 4 md)
...
| **UI/UX 폴리싱 C-2~C-7** | 선택지 rest 어포던스(.choice-btn 골드 카드) · 시나리오 카드 배너(getScenarioBannerImage, fallback 그라데이션) · 스탯 뮤트 앤틱 팔레트(--stat-* 토큰, 중복 3곳 정본 수렴) · 라벨 정리(카리스마 줄바꿈·레이더 한글·범례 제3명칭 통일) · 모바일 메뉴 lucide · 골드 체크박스(.checkbox-gold). C-1(HUB 자유 입력)은 기획 판단 대기 — architecture/68 부록 A | ✅ 완료 |
...
### architecture/ — 통합 아키텍처 (55 md + INDEX)
