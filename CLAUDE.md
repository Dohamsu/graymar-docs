...
| **이벤트-서술 NPC 분열 (부록 L)** | 버그 185a8ddd — 첫 진입 WORLD_EVENT로 음유시인 조우 이벤트 매칭, 서술은 정보상·선택지는 음유시인 분열. EVT_TAVERN_ENC_BARD primaryNpcId 명시(콘텐츠) + 유저 명시 지목≠이벤트 NPC 시 이벤트 선택지 폐기 게이트(코드). 조우 이벤트 NPC 명시 규약은 후속 — architecture/68 부록 L | ✅ 완료 |
