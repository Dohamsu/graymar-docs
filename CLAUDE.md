...
| 108_natural_equipment_acquisition.md | 📎 설계→구현 | 자연스러운 장비 획득 경로 — 30일 1,102턴 실측에서 드랍 자격 턴이 9.4%뿐이고 지배적 플레이(TALK+INVESTIGATE+PERSUADE 61%)가 획득에서 배제되던 역설. **A** 관계 선물(`npcs.json gift`, trust≥30) · **B** 단서 연계 은닉 장비(`facts.json cache`) · **C** NPC 개인 거래(`personalTrade`, bribeOpportunity 미러·골드 소비처) + **대화 턴 랜덤 드랍 제거**(`EQUIPMENT_DROP_ACTIONS`에서 HELP 제외 — 장비만, 골드 사례는 유지). 4계약: 서버 결정론(불변식 1)·런당 1회(파밍 불가)·`[이번 턴 획득 아이템]` 서술 정합(arch/40)·콘텐츠 저작(불변식 45). audit_content 모양 검사 3종으로 사문 배선 방지 |
