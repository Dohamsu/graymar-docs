...
15. **NPC 이름 비공개→공개** — FRIENDLY·FEARFUL 1회 / CAUTIOUS 2회 / CALCULATING·HOSTILE 3회 소개. 2턴 분리: 소개 턴엔 별칭 유지(LLM이 이름 밝힘 연출), 다음 턴부터 실명 — 모든 소개 경로는 `introducedAtTurn` 필수. LLM이 이름을 언급하지 않으면 IntroRollback으로 취소되며, 같은 턴 재소개 금지(한 턴 소개 시도 1회) — architecture/64.
...
| **NPC 이름 공개 무결성** | 롤백-재소개 상쇄 차단(한 턴 소개 1회) + 표시명 turnNo 일괄(2턴 분리) + 소개 힌트 실명 콘텐츠 직접 + injected introducedAtTurn + 별칭 접두 중복 2중 정리. 핍 케이스 실증·실런 검증, 회귀 11건 — architecture/64 | ✅ 완료 |
