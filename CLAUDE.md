...
## 이미지 생성 정책 (2026-09-10 소유자 지시)

**프로젝트 코드·스크립트·셸에서 이미지 생성 API를 직접 호출하는 것은 금지한다.**
Gemini·DALL·E·Stable Diffusion·OpenRouter 등 공급자와 관계없이 curl, 임시 스크립트,
프로젝트 실행 코드, 서버 엔드포인트를 통한 생성은 허용하지 않는다.

- **허용 예외:** 소유자가 이미지 제작을 요청한 경우 Codex가 제공하는 **내장 이미지 생성 도구**는
  사용할 수 있다. 이는 프로젝트 런타임이나 프로젝트 API 키를 이용하는 생성과 구분한다.
- 게임 에셋 이미지(장면 컷·초상화·장소 등)는 내장 도구로 시안을 만들 수 있지만, 생성 즉시
  게임에 반영하지 않는다. 먼저 소유자의 확인을 받고, 승인된 결과만 sync·등록·매칭한다.
- 여러 이미지를 제작할 때는 먼저 대표 시안 **2장 이내**를 만들고 방향을 확인받은 뒤 나머지를
  생성한다. 소유자가 한 번에 전체 생성을 명시적으로 요청한 경우에는 그 요청을 따른다.
- 서버 `scene-image` 모듈(Gemini 장면 이미지)은 **봉인 유지** — 에이전트가 봉인 해제·호출·
  활성화하지 않는다. 해제는 소유자의 명시적 별도 지시로만 가능하다.
- 테스트용·플레이스홀더 이미지도 프로젝트 코드나 외부 API로 생성하지 않는다. 내장 이미지 생성
  도구 사용 여부는 위의 소유자 요청·검토 절차를 동일하게 따른다.
...
**초대 게이트 계약도 함께 검증**한다 (arch/107 §8) — 게이트가 ON 이면 코드 없는 가입이
거절되는지 직접 찌르고, OFF 면 경고를 띄운다. "비공개 테스트가 조용히 풀린" 상태가
정확히 스모크가 담당하는 부류라서다. 신규 계정 생성에 초대 코드가 필요하므로
**`ADMIN_TOKEN` 이 환경에 있어야 한다** (없으면 `server/.env` 에서 읽는다 — `invite_util.py`
· `e2e/_helpers.ts` 가 1회용 코드를 자동 발급).
...
- **초대 게이트 (arch/107 §8)**: 비공개 테스트 중이라 신규 가입에 초대 코드가 필요하다. 정본 테스터 계정은 이미 있어 login fallback 으로 빠지지만 `--new-account` 는 코드가 필요하다. 발급 정본은 **`scripts/invite_util.py`** (`add_invite_code(body, BASE)`) — 가입을 시도하는 모든 스크립트가 이걸 쓴다. `ADMIN_TOKEN` 은 env 또는 `server/.env` 에서 읽는다. **서버에 테스터 예외를 두지 않는다** (구멍을 만들면 게이트가 무의미).
- **V12 프롬프트 예산 게이트는 ADMIN_TOKEN 필요 (2026-09-08)**: `includeDebug` 의 `llmPrompt` 전문이 `x-admin-token` 동반 요청에만 열리므로(보안 감사 M1) `playtest.py` 가 env/`server/.env` 의 `ADMIN_TOKEN` 을 첨부한다. 없으면 V12 표본 0 으로 게이트가 보류된다. 테스터 비밀번호 정본은 `server/.env PLAYTEST_PASSWORD` — 스크립트·스킬·문서에 리터럴을 두지 않는다(`scripts/playtest_env.playtest_password()`).
...
├── content/             ← 게임 콘텐츠 시드 데이터 (graymar_v1 + star_sand_v1 별빛모래 + karnholt_v1 자율 서사 AUTONOMOUS 팩 — **2026-09-03 `status: HOLD`** 홀딩: 목록·런 생성 제외, 기존 런 조회 유지 (silverdeen_v1 은 2026-08-18 제거))
...
### Server — 16 modules, 123 services, 21 controllers (2026-09-09 `@Injectable` grep 기준)

| 모듈 | 서비스 수 | 역할 |
|------|----------|------|
| common/ | - | Guards(AuthGuard·AdminGuard), Filters, Pipes, Decorators(@AdminEndpoint·@AllowQueryToken·@UserId), dialogue-act·tester.util |
| auth/ | 1 | JWT 인증 (register/login/logout/me/withdraw, httpOnly 쿠키 세션) |
...
| turns/ | 10 | 턴 제출/조회 라우터 + 도메인 서브서비스 9종 (TurnShared·EquipShopTurn·DagTurn·HubTurn·CombatTurn·RelationTurn — arch/77 §18·§19 / **LocationTurn·LocationQuest·LocationResult — arch/112**, 2026-09-02) |
| llm/ | 26 | Worker(+worker/ NarrativePostprocess·RunStatePatch — arch/114), ContextBuilder, TokenBudget, Prompt, NpcDialogueMarker, NanoDirector, NanoEventDirector, NpcReactionDirector, ChallengeClassifier, ThemeClassifier, DialogueGenerator, LlmStreamBroker, StreamClassifier, SceneCutMatcher, FactExtractor, Lorebook, MemoryRenderer, PlotDirector, PlotSeedGenerator, LlmCallLog 외 |
...
| LLM | **[2026-09-03 개발 기간] 메인 = Upstage Solar Pro 4 전턴, 교차 중단 (arch/113 — 프로파일 자동 적용, 원복은 .env 두 줄)** · 이전 정본: Gemma 4 31B dense (메인, stream:true, provider allowlist Friendli·Venice·Novita·CoreWeave) / **gpt-5.6-luna** (5:5 교차 — 짝수 턴. 2026-08-13 DeepSeek 교체: 어체 위반 43~46%→6.4%, 마커 커버리지 75%→100% 실측) / GPT-4.1 Mini (fallback) / GPT-4.1-nano (경량) | Multi-provider via OpenRouter (arch/25 부록 D·D-8) |
...
2. **LLM is narrative-only (하드 상태 한정)** — LLM은 **하드 상태**(HP·골드·인벤토리·questState·판정 결과)를 절대 쓸 수 없다. 실패해도 게임 진행 (턴은 LLM 호출 전에 커밋, llmStatus 게이트 없음). 단 **소프트 상태**는 나노 LLM 출력이 `applyRunStatePatch` CAS(fresh 재조회 + jsonb 낙관적 잠금, llm-worker.service.ts) **경유로만** 역류가 허용된 설계된 회색지대다: NPC 감정 블렌드(emotionalShiftHint, arch/76 D3-b′)·작별 감지(npcFarewell)·소개 성사(NpcAppearance)·테마 기록·nextBeats·propsTrace·장면 컷 상태(sceneCutState, arch/96)·관계 신호(relationSignal, arch/109 — nano 는 신호 감지만, 전이 판정은 `relationship-kind.core.ts` 순수 함수·수락 조건은 서버 임계+콘텐츠 romanceable). 새 역류 경로 추가 시 ① 하드 상태 금지 ② CAS 경유 ③ 이 목록 등재의 3조건을 지킨다. **타입 벽(arch/114 T3, 2026-09-08)**: CAS 단일 쓰기 지점은 `llm/worker/run-state-patch.service.ts` 의 `RunStatePatchService.apply` 이며 patch 콜백은 `SoftStateView`(`db/types/soft-state.ts` — npcStates·actionHistory·sceneCutState·nextBeatCandidates·relationships·worldState{companionNpcId,locationDynamicStates}·characterName 읽기 전용)만 본다. `rs.hp`·`rs.gold`·`rs.questState` 접근은 컴파일 오류(`pnpm typecheck:soft-state`, 스펙 `run-state-patch.soft-state.spec.ts` 의 `@ts-expect-error` 가 고정). ③ 등재 = 이 타입에 필드 추가. 단 `noImplicitAny: false` 라 대괄호 문자열 접근은 미보증 — 리뷰에서 막는다.
...
17. **Token Budget 2단 (2026-09-01 백스톱 20,000자 상향 — 감시 조건부)** — 메모리 블록 2500(블록별 배분·저우선 트리밍) + 프롬프트 총량 백스톱 `GRAND_TOTAL_CHAR_BUDGET` **20,000자**(≈12.2k tok). 구 16,500자(11k tok soft 지시 절벽 실측)에서 상향한 근거: 30일 실측(llm_speech_audit 1,380턴)에서 길이 구간별 어체 위반율이 구 상한까지 평탄(Gemma 11~16%·Luna 12k+ 8~15%) + Gemma 4 31B 장문 여력(RULER@128k 96.4%). 구 절벽 실측은 DeepSeek 교차 시절 산물로 판단(소유자 결정). **감시 조건**: 신규 개방 밴드(>`GRAND_TOTAL_WATCH_CHARS` 16,500자)는 `llm_token_stats`(promptChars·budgetTrimmed) 텔레메트리 + **정본 `scripts/prompt_budget_monitor.py`**(밴드 위반율 기준 대비 +5%p = 롤백 신호, 특히 Luna — 패밀리 장문 약체 MRCR 41.3%)로 상시 관측. 백스톱 제거 순서는 [세계 상태] → [NPC 일상] → 기억 부분 절삭 — 기억·L0 절대 보호. **백스톱은 하드 상한이 아니다** — 제거 대상 블록이 없거나 작으면 초과분이 그대로 남는다 (arch/101 자기점검 1회차). 재비대는 플레이테스트 V12 게이트(발동 프록시 19,500자·발동률 ≤20%·**avg ≤15,000자 유지** — 상향이 재비대 면허가 되지 않도록, 원장 v2 리셋)가 감시 — arch/79 2·3차.
...
27. **NPC knownFacts 점진 공개 + 기록·서술 단일화** — SUCCESS/PARTIAL 판정 + 정보행동 시 NPC 보유 fact 중 **입력 주제 매칭 우선**(없으면 순서) 공개. 발견 fact는 `ui.questReveal`로 LLM 서술에 동일 주입되어 기록 fact = 서술 fact 보장 (architecture/58). 이벤트 discoverableFact는 SUCCESS=100%, PARTIAL=50%. FAIL은 미공개. **이벤트 경로 FREE 대화 게이트(2026-09-08 QC3)**: ChallengeClassifier FREE(자동 SUCCESS) 턴이 대화 계열(TALK/PERSUADE/HELP/TRADE/REST/SHOP)이고 입력이 fact `keywords` 에 닿지 않으면 이벤트 discoverableFact 를 공개하지 않는다 — 불변식 44 를 이벤트 경로에 적용(기행·잡담 턴이 자동 SUCCESS 라는 이유로 단서를 100% 낙하시켰다). 상황 생성기 활동 이벤트의 discoverableFact 는 스케줄 NPC 가 보유(knownFacts/knownBy)한 것만 싣는다. FREE 턴은 서술 주입 게이트에서 SUCCESS 로 간주(context-builder `factOutcome`)해 기록=서술을 유지한다 — arch/58 §5.
...
40. **자유 행동 주사위 스킵** — ChallengeClassifier가 룰 게이트(NON_CHALLENGE/ALWAYS_CHALLENGE) + 회색지대 nano 분류로 FREE/CHECK 결정. FREE면 주사위 스킵 + 자동 SUCCESS. **CHOICE 는 `turns/choice-challenge.core`** (STRUCTURAL_FREE → ALWAYS_CHECK → fact 판돈 → BLOCK → riskLevel≥2 → 라벨 판돈 → 저작 INVESTIGATE/SEARCH → FREE). **저작 접근 선택지 면제(2026-09-08 QC4~6)**: PERSUADE 이면서 nano 가 아닌 저작 선택지에 `payload.riskLevel: 1` 이 명시되면 ALWAYS_CHECK 를 건너뛰어 판돈이 없을 때 FREE — "주민에게 말을 건다" 류 접근 대화 9종(scene-shell)+콘텐츠 `encounter_talk`. **affordance 는 이벤트 매칭 축이라 TALK 로 바꾸지 않는다**(저작 이벤트 funnel 30일 72%) — 판정·톤은 `choice-challenge.core`·`tone-hint.core` 에서 푼다.
...
46. **사례금 = 런 종료 단일 정산 → 다회차 이월 자원 (C 전환 2026-08-31, 소유자 결정)** — fact 발견/questState 전환 시 quest.json `rewards`(factGold/transitionGold)가 **적립**(`RunState.pendingQuestReward`, 총량은 콘텐츠로 유한 → 파밍 불가)되는 것은 유지하되, **실지급은 런 종료(RUN_END) 1회뿐**이다. 구 3트리거(의뢰인 대면·거점 복귀)는 폐지 — 30일 실측 유입:유출 **12:1**(소비 16건 전부 BRIBE, 상점 구매 0)로 경제 루프가 성립하지 않았고, 중간 정산 연출은 만성 마찰(서랍 로넨·그림자 대리인·의뢰인 편재 — 연출 가이드 2회 수정으로도 재발)이었다. 최종 골드는 캠페인 이월(`carryOverRules.goldRate`, runs.service)로 다음 시나리오에 연결되는 **다회차(로그라이크) 자원**이 정체성이다. 적립 이벤트(`QUEST_REWARD_ACCRUE`)의 LLM 프롬프트 제외는 유지(주체 없는 금액 주입 = 임의 귀속 실측 35%), [사례금 정산 연출] 프롬프트 채널은 제거. BRIBE 비용은 `quest-balance.config.ts`(-6/-3), 재원은 시작 골드(20~80) — architecture/65 + 89 (C 전환).
47. **디렉터 비트 = 의도 정합 시에만 채택 (불변식 D)** — 자율 서사(AUTONOMOUS) 디렉터의 선계산 비트는 **플레이어 의도와 정합할 때만** 채택한다. 인력(gravity)은 유인이지 강제가 아니다 — 서사 강제 진행 금지. 구체: 강제창(`BEAT_FORCE_AFTER_TURNS`, turns.service `determineTurnModeCore` 규칙 1.5-C)은 **대화 잠금 활성 턴·사교 발화(GREETING/WELLBEING/THANKS/FAREWELL)·REST 의도 턴에는 발동하지 않는다**. **상호작용 단위 확장(버그 d20c1de8, 2026-07-17)**: 직전 턴과 동일 NPC 연속 상호작용(contextNpcId — 사교든 폭력이든) 중이면 그 NPC를 포함하지 않는 비트는 승격(1.5·3.6)·채택(requiredNpcId 하드 게이트) 모두 금지 — 채택 비트가 화자를 가로채 구타 대상이 스왑되던 실측 차단. **무명 화자 확장(2026-09-01 QC4)**: contextNpcId 가드는 무명 화자(primaryNpcId=null)를 보호하지 못하므로, 직전·이번 턴이 모두 대화 계열(RENEW)이고 명시 지목이 없으면 비트 채택 금지 — 무명 정보원 위협 연속 중 강제창이 장면에 없는 NPC를 화자로 소환하던 실측(run 27c54362 T7) 차단. 탐색(HOLD) 경로는 유지. "대화·휴식하려는데 사건 끼워넣기" 패턴이 조사 최다 이탈 요인("의도 무시 강제 진행")이므로 원천 차단. 정합률은 P8 계측(의도 정합 채택률)으로 감시 — architecture/76 D1.
...
52. **경계 거절은 3부류 + 성과 차단(B안 2026-08-21)** — 무대 이탈(이동, `move-boundary.core`) 외에 **META**(게임 종료·수치 조작·로그아웃 — 세계 안에서 실행 자체가 없음 → `OUT_OF_SCOPE` + 주사위 스킵 + `[세계 밖 요청 지시]`)와 **ANACHRONISM**(시대착오 물품 — 물건만 없음 → `IMPLAUSIBLE` + FREE 강제 + `[행동 재해석 지시]`)를 `turns/world-boundary.core.ts` 가 담당한다. **plausibility가 IMPLAUSIBLE·OUT_OF_SCOPE인 턴은 `forceNoGain`**(`isWorldBoundaryNoGain`) — 재해석 서술은 흐르되 모든 델타 0·fact 미공개(outcome FAIL)·판정 배너와 resolveSkipped 칩 미표시·요약 "통하지 않았다". 구 동작은 FREE→자동 SUCCESS라 "허튼 입력이 일상 행동 성공으로 포상"됐다(리뷰 제안 5). refused(세이프티)와 달리 **과금은 유지**(무과금이면 허튼 입력 반복이 LLM 비용 어뷰징 벡터). L1 룰 목록은 안전망이고 일반해는 L2(nano plausibility 루브릭) — 목록 확장은 whack-a-mole 이므로 L2 를 먼저 손본다. 오탐 통제: 한국어 일반 단어에 부분 매칭될 토큰(`총`→총각, `전기`→전기수, `시계`→시계탑)은 등재 금지, 복합어만 — architecture/105.

53. **분위기(toneHint)는 장면 성질에서 도출** — 판정 결과의 파생값으로 두면 축이 죽는다. 구 구현(`SUCCESS → triumph`)은 30일 LOCATION 2,162턴에서 `triumph` 57%를 만들고 `tense`·`calm`·`mysterious` 를 **한 번도 쓰지 않았다**(6값 중 3개 사문, 잠입 8턴 내내 승리감 주입). 정본은 `turns/tone-hint.core.ts` — 세계 밖 시도 → 위험 → 긴장 행동 → 경계 상태 → 사교 → 사회적 성취 → 탐색 → 시간대 순. `triumph` 는 상대를 넘어선 성취(PERSUADE·BRIBE·HELP 성공)에만, 단서 발견은 `mysterious`. **FREE 사회 행동은 `calm`(2026-09-08)** — `resolveSkipped`(주사위 스킵)+PERSUADE/BRIBE/HELP 는 판정 없이 승리감을 주입하지 않는다. **부산물이 아니라 의도를 신호로 삼는다**(불변식 44 잡담 게이트와 같은 교훈) — architecture/105.
...
| POST | `/v1/auth/logout` | httpOnly 인증 쿠키 제거 (무인증·멱등 — 클라 쿠키 세션 전환 대응, 보안 감사 2026-09-07 M4) |
...
| GET | `/v1/runs/:runId/turns/:turnNo` | 턴 상세 (LLM 폴링용, includeDebug 옵션 — `llmPrompt` 전문은 `x-admin-token` 동반 요청에만, 보안 감사 M1) |
| GET | `/v1/runs/:runId/turns/:turnNo/stream` | **SSE** 서술 스트리밍 (arch/35, `?token=` 쿼리 인증은 `@AllowQueryToken` SSE 2곳 한정 — 보안 감사 M3) |
...
| GET | `/v1/parties/:partyId/stream` | **SSE** 실시간 스트림 (?token=JWT — `@AllowQueryToken` 한정) |
...
OPENAI_API_KEY=sk-...
OPENAI_MODEL=google/gemma-4-31b-it   # OpenRouter 메인 모델 (Gemma 4 31B dense — 2026-07-22 승격, allowlist 전제. 이전 26B MoE). **[2026-09-03 개발 기간] 실제 .env 는 upstage/solar-pro4 + LLM_ALTERNATE_MODEL 주석(교차 중단) — arch/113 §6, 원복 두 줄**
LLM_PROMPT_PROFILE_MAP=                # 모델별 프롬프트 프로파일 강제 (`모델=solar;…`). 미설정 시 슬러그에 solar 가 있으면 자동 (arch/113)
...
LLM_REASONING_EFFORT_MAP=openai/gpt-5.6-luna=none  # 모델별 추론 강도 (`모델=값;모델2=값`). Luna는 default_effort=medium 이라 미지정 시 추론이 켜진 채 출력 단가로 과금. **[2026-08-27] 모델 계열 무관 적용** — 이전엔 OpenAI 추론 계열이 아니면 맵을 읽기도 전에 빠져나가, 추론이 기본 ON 인 비-OpenAI 모델을 길들일 방법이 없었다 (GLM 5.3 Flash: 미지정 시 default max 로 추론이 max_tokens 를 소진해 **본문 0자**). 맵에 없는 모델의 기본 동작은 종전과 동일(OpenAI 추론 계열 none, 그 외 무필드). 수용 값 집합은 모델마다 다르므로(Luna none~max / GLM max·high·low) 판정은 프로바이더에 위임 — arch/25 부록 G
...
ADMIN_TOKEN=<secret>                  # 어드민 헤더 인증 x-admin-token (arch/87, JWT+users.role 과 OR). 플레이테스트 V12 게이트(includeDebug llmPrompt)도 이 토큰을 첨부
HOST=127.0.0.1                        # 바인딩 호스트 — 기본 루프백(cloudflared 만 접속). LAN 직결 테스트 시 0.0.0.0 명시. trust proxy 는 'loopback' 고정 (보안 감사 2026-09-07 M2)
POSTGRES_PASSWORD=<secret>            # docker-compose 가 참조(미설정 시 기동 실패) — DATABASE_URL 비밀번호와 동일하게. 회전은 TODOS.md 소유자 작업
PLAYTEST_PASSWORD=<secret>            # 정본 테스터 playtest@test.com 비밀번호 — 스크립트는 scripts/playtest_env.py·e2e/_helpers.ts 로만 읽는다 (리터럴 금지)
...
| **이미지 정본 단일화 (2026-08-14)** | [arch/102] 장소 이미지 결정이 100% 클라 하드코딩이라 장면 컷 장소 후보가 graymar·star_sand에서 영구 0이던 결함 해소 — location_images.json 콘텐츠 승격(URL 불변, 매처 스펙 17케이스) + 초상화 npcs.json portraitUrl 외부화(클라 미러 맵 삭제) + 마커 와이어 포맷 URL→npcId 전환(혼재 허용) + 장소 이미지 URL 서버 이관. **잔여 마감(2026-08-31)**: 클라 레거시 리졸버 삭제(14일 767/767 커버 실측 후) + hubImage 콘텐츠 승격으로 HUB 전경까지 서버 정본화(비-graymar HUB의 graymar 전경 오염 동시 해소) | ✅ 완료 |
...
| **상품화 점검 + 법적 고지·비공개 전환 (2026-08-20)** | [arch/107] 상품화 준비도를 실측으로 점검. **실유저 18명(오너 제외)·재방문 1명·엔딩 도달 0명·17일째 신규 0** — 병목은 콘텐츠가 아니라 유입과 리텐션이고, 인프라는 개인 Mac(launchd+cloudflared+로컬 Docker PG, **백업 0**)·PG 미연동·법적 고지 0건이었다. **P0-2 수행**: 이용약관·개인정보처리방침 신설(`/terms`·`/privacy`) + 표시사항 정본 `operator.ts`(리터럴 금지, 불변식 45 와 같은 이유) + 가입 필수 동의. 방침 전 항목을 코드·DB 로 실측해 작성 — **쿠키 0건**·localStorage 5종·`turns.raw_input` 의 **국외 이전**(OpenRouter→Friendli·Venice·Novita·CoreWeave, Vercel, Cloudflare)이 핵심 고지. **비공개 테스트 전환**: 초대 코드 가입 게이트(`invite.core`/`invite.service`, `INVITE_CODE_REQUIRED` **fail-closed** — 명시적 false 만 개방, 코드 클레임+유저 생성 단일 트랜잭션으로 코드 유실 차단) + 랜딩 초대 요청 동선. **색인 차단은 착수했다가 철회** — 등급분류 기준은 "유통·이용 제공"이고 시험용 게임물 제도도 기간(60일)·인원(2만)으로 규정할 뿐 색인을 보지 않는다. 실질 방어는 게이트 하나이며, 유입이 0인 상황에서 색인을 닫으면 초대 요청 리드 채널만 잃는다. `/play` 만 차단 유지 + 메타 카피를 초대제로 정렬. **함정 2건**: page metadata 는 layout 을 **덮어써서**(병합 아님) 전역 noindex 가 정작 유일한 색인 페이지인 랜딩에 안 걸림 / 게이트가 스모크·플레이테스트의 신규 계정 생성을 막아 **서버 예외 대신 스크립트가 ADMIN_TOKEN 으로 1회용 코드를 발급**하도록 수정. **규제 검토**: 게임제작업 등록(제25조, 미등록 = **2년 이하 징역 또는 2천만원 이하 벌금**)·게임물 등급분류(제21조)가 앱인토스 진입의 하드 게이트이자 현재 미준수. §7 유사 서비스 실측 — GPTRPG·프론티아·크랙·제타 **전원 등급분류 미표시**, "엔터테인먼트 카테고리 + 자체 심의 규정"이 de facto 표준. 다만 게임위가 든 게임성 요소(유료 재화·확률·획득/손실)를 dimtale 은 전부 갖춰 캐릭터챗보다 먼저 걸리는 쪽. **검증**: 서버 2,194 passed·스모크 PASS·게이트 거절 3종 실측·색인 정책 4라우트 확인. **잔여(§9 TODO)**: `operator.ts` contactEmail 공란(랜딩 CTA 비활성)·구청 자택 등록 가능 확인·사업자등록→게임제작업 (회원 탈퇴는 2026-08-31 구현 — 아래 항목) | ✅ 완료 |
| **NPC 관계 엔진 R1~R3 (2026-08-26)** | [arch/109] 관계 유형(FRIEND/CONFIDANT/ROMANCE/RIVAL/GRUDGE)·마일스톤을 서버 실상태로 신설 — 전이 정본 `relationship-kind.core.ts` 순수 함수(수락=romanceable+BONDED+fear<40+susp<50, 거절 쿨다운 5턴), nano relationSignal 은 감지만(불변식 2 등재), 쓰기는 워커 CAS 단일 지점. 프롬프트는 순증 0(티어 힌트·관계 깊이·R4 호칭 값 교체). 콘텐츠 romanceable 저작 17명(명시 false 4명 — 기혼·성직·미성년) + audit 모양 검사. R2: 잡담 관계 소재(12턴 window)·제3자 인지 `[주변의 눈]`. R3: 인물 탭 관계 배지·관계 기록(라벨 정본 서버). **실런 게이트 PASS**(고백 수락→kind=ROMANCE 영속→재개 재회 연인 톤+이름 호명). 게이트가 잡은 결함 2건: 고백 THREATEN 오분류+nano NONE 이중 소실(L1 `detectConfessionIntentL1` 신설 — **실측에서 nano 는 두 번 다 NONE, 성사는 L1 폴백**), 연인 재회 "손님" 호칭(R4 값 교체). 잔여: 아카이브 관계 요약·GRUDGE 실런 | ✅ 완료 |
| **선택지 전달 결함 + 전진·복귀 동선 (2026-08-21)** | 플레이어 리뷰("둘러본다/조사한다 로테이션") 재현 디버깅 — 진범은 nano 생성이 아니라 **전달**: 복원(이어하기)이 llm_choices를 버리고 serverResult placeholder를 렌더(3중 결함 — GET runs 턴 이력 llmChoices 미포함·applyRunSnapshot placeholder 사용·mapResultToMessages의 CHOICE 메시지가 live-choices 합성을 가림). 라이브 스트림 교체는 5/5 정상 실측. 수정: ① 턴 이력 llmChoices 노출 + 복원 nano 우선 + CHOICE 메시지 제거(단일 소스화) ② CHOICE 조회 최근 3턴 확대 + 미해결 `[ChoiceLookupMiss]` 경고(stale id가 자유텍스트로 강등되던 것 계측) + stale 블록 disabled 가드 ③ **quest_forward 전진 선택지**(`quest-forward-choice.core`) — 정체 3턴 + 다음 미발견 fact가 타 장소면 MOVE_LOCATION 선택지 주입(payload.targetLocationId 명시 이동, 워커 nano 교체본 보존, 도착지=현위치 가드) ④ **go_hub 게이트**(`go-hub-gate.core`) — 거점 복귀 선택지를 사유 턴(단서 발견·단계 전환·HP≤35%·정체 3턴·파티)에만 노출, 진입 턴 미노출, 워커 goHubChoiceFor 서버 에코 전용화. 탈출구 상시 유지(데스크톱 헤더·모바일 메뉴 신설·자유 입력·go_ 접두 직접 제출). 실측: 복원 nano_31_* 표시·quest_forward_36 발화→LOC_MARKET 이동·배포 후 스모크 런 진입 턴 go_hub 없음/단서 턴 노출. 스펙 24(코어 2종) | ✅ 완료 |
| **GLM 5.3 Flash 후보 평가 (2026-08-27)** | [arch/25 부록 G] 출시 익일 모델 조사·환경 세팅. **보류** — 비용 동급(1.30 vs Gemma 1.32원/턴)인데 라벨 준수가 **66.7~76.3%**로 DeepSeek 탈락 구간이고 실패 양상도 같다(화자 없는 배경 대사를 서술에 인라인). 실프롬프트 15건 재생 + 같은 세션 대조군. **진짜 소득은 세 가지 함정**: ① 추론이 mandatory 인데 기본값이 max — 그냥 부르면 추론이 max_tokens 를 소진해 **본문 0자/29초**, 스트리밍은 콘텐츠 델타 자체가 없어 전 턴 fallback 강등(끄는 게 아니라 낮추는 게 답) ② **`resolveReasoningEffort` 가 OpenAI 추론 계열이 아니면 env 맵을 읽기도 전에 빠져나가** 비-OpenAI 모델을 운영값으로 길들일 방법이 없었다 → 계열 무관 적용으로 일반화(F-4 Gemini 계열 분기 땜질의 일반해) ③ **프로바이더가 속도·품질을 둘 다 갈랐다** — 같은 8건에서 Novita 18.4s/66.7% vs BaseTen 7.1s/100%, penalty+require_parameters 가 저가 티어를 404로 걷어내 Novita 만 남음. 하네스 보강: `extract-bench-prompts.py` 신설(추출 단계가 아예 없었다·런 테이블은 `run_sessions`)·`llm-config-ab.py --only`(결론을 뒤집은 스위치). 신규 스펙 5·전체 2,370 passed. **미검증**: 라이브 파이프라인(오프라인 재생만), BaseTen 라벨은 429로 n=5~8 | ✅ 완료 (도입 보류) |
| **품질 사이클 시리즈 + 실유저 버그 5ae9b872 (2026-09-01~02)** | `/quality-cycle` 19회차(3팩×페르소나 6종, 회차당 12턴) — 결함 계보 5축(장면 경계·실명 치환·라벨 조립·디렉터/행동화·후처리 위생) 서버 커밋 16 + 콘텐츠/감사 5. 게이트가 잡은 결함 0, 전부 정독. 후속: V9 주제 집중 오탐 완화·되묻기 억제·미완결 등장 교정·fact.topic 라벨 소재 상류 전환(001418a). 익일 버그 5ae9b872("여관 이동 선택지가 안 먹음") — nano 이동 라벨이 TALK affordance 로 실려 대화 턴 처리되던 것을 `nano-move-choice.core` 결정론 승격(MOVE_LOCATION+targetLocationId)으로 해소(30일 라벨 3,919건 재생 오탐 0). 검증런 2회 정독에서 **QC16 재제공 차단이 공허한 수정**(actionHistory 마지막 = 한 턴 전과 비교)이었던 것과 **sceneFrame 24자 절단이 꼬리 변형 7종의 한 뿌리**였던 것을 잡아 rawInput 비교·표제형 명사구 게이트(`sceneFrameToContextCore`, 3팩 203건 전수 재생)로 교정. 하네스 관찰: 에이전트는 서버 기본 선택지에서 고르고 실클라는 nano 우선 — `playtest-reports/qc_series_20260901_summary.md` §5~7 | ✅ 완료 |
| **대화 진전 구조 — 빈손 대화 가드 + 콘텐츠 보유 동기화 (2026-09-02)** | 20턴 롱런 정독: 만난 NPC 셋 전원 knownFacts 0 이라 "잘 이어지지만 진전 없는" 대화(30일 대화 턴 43% 빈손 화자). ① `empty-handed-hint.core` — 같은 NPC 2턴째·보유 fact 0 이면 "모른다 → 다음 단서 보유자 지목"(현장 보유자 선택지 보장 / 타 장소 quest_forward 2턴째) ② 배경 화자 턴 nano 선택지 다양화(`nano-bg-diversify.core`) ③ 어체 가이드 호칭 콘텐츠 우선(R4 vs 어체 기본 호칭 자기모순) ④ 인계 topic = fact.topic. 콘텐츠: facts.json knownBy↔npcs.json knownFacts drift 7건 동기화(저작돼 있으나 엔진이 못 읽던 사문) + SUB 5명 부분 보유 versions(소유자 승인) + L0 테마 결말 문장 제거. 감사 규칙 3종. 플레이테스트 하네스는 nano 선택지 우선(실유저 표면) — `playtest-reports/qc_series_20260901_summary.md` | ✅ 완료 |
| **Solar 전턴 품질 사이클 9회차 + 후속 3건 (2026-09-03)** | `/quality-cycle` 9회(graymar·star_sand·karnholt, 페르소나 6종) — 게이트가 잡은 결함 0, 정독에서 서버 수정 16건: 다어절 fact 키워드 사문(`extractFactQueryKeywords`)·질문의 이동 오판(`hasMoveSuffixAfter` 이동 동사 필수 + 잠금 중 의문형)·저장본 화자 연속 귀속(regex/콜론/consecutive 4경로 + 다른 화자 지목 가드)·대사 축자·접두 재탕 제거(`dialogue-repeat.core`)·대사 전멸 재귀속(`[FocusedAuxReassign]`)·꼬리 칭호 별칭 경계·플롯 시드 이름 재사용 검증·따옴표 안 마커 언랩·역할 라벨 무명화·영단어 잔해. 후속(소유자 결정): ① **karnholt `status: HOLD`**(저작 NPC schedule 부재 → 화자 없는 턴 46.5%, `listAvailableScenarios` 제외 + createRun 거부 + 랜딩 카드 숨김) ② **[단서 방향] 힌트 문장 → 조각 주입**(`quest-hint-fragments.core` — 장소·인물·열쇠말만, 원문 낭독 8런 중 2런 근절 목적) ③ **어체 v3** — R5v2 맵 확장(HAOCHE 겠지·구만 / HAECHE 신설 / GYUSU 시오·다오·구려 / HAEYO 하게체 세·게나) + 감사기 말끝 흐림(미완 절 마지막 문장은 앞 문장으로 판정). 보고서 `playtest-reports/qc_solar_20260903_summary.md` | ✅ 완료 (①~③ server 3055b51·8871438) |
| **서술 모델 후보 탐색 → Solar Pro 4 프로파일 → 전턴 운영 전환 (2026-09-03)** | Gemini 3.8 Flash 조사(추론 off 불가·temperature 폐지·17~35원/턴 → 채택 불가, arch/25 H) → 후보 9종 탐색(I, 10원 상한 확장) → Solar Pro 3 탈락(라벨 0~14%)·Solar Pro 4 재생 40건 라벨 57%(J-5) → 재검토: 라벨 재강조 80.4%, 와이어 마커 역효과, 실패 원인은 문법이 아니라 인라인 산문 습관(J-6) → **arch/113 모델 프로파일**(접두/접미 + 화자 연속 귀속) 실런 라벨 70→82·어체 16.7→8.1·무라벨 3→0 → 전문 정독(§5: 문장력·화자 규율 Solar 우위, 어체·문법 Gemma 우위, Luna 는 짧고 얕음) → 소유자 결정으로 **병합 + 메인 모델 Solar 전턴·교차 중단(개발 기간)** + v2(대명사·호칭·화폐 재강조, 다중 인용 분리) + **로어북 미발견 fact 세부 게이트**(기록 없는 공개 채널 3호, 모델 무관). 원복은 .env 두 줄 | 🔄 운영 중 |
| **30턴 롱런 정밀 분석 → 결함 3건 수정 (2026-09-02)** | run 792a8c98(graymar·chatty) 전문 정독 + 30일 DB 재집계 — `playtest-reports/run_20260902_long30_analysis.md`. **A 아크 3막 무커밋 진행**: `getNextArcStage` 가 커밋을 `currentRoute` 로 근사했는데 ARC_HINT 이벤트가 커밋 전에도 채우는 필드라, 커밋 0회 런이 T21·28·29 에 안내+완수 동시 처리 후 S1 에서 엔딩(30일 5/11런 무커밋 진행·미완 엔딩 2). `ArcState.committedAt` + `isArcCommitted`(레거시 commitment≥2) + 안내 턴 완수 금지(`isArcStageAnnounced`) — arch/103 §6. **B nano `[정보 전달]` 유령 지시**: nano 가 답한 `factRevealed` 를 서버 대조 없이 하드 지시로 승격(30일 무공개 턴 676/2,016 주입·FAIL 턴 36%, 단서형 어휘 2.4배) → 범인이 종반부 증거를 창작 발화. `nano-fact-reveal.core` 로 `ui.questReveal` 정렬 + 이벤트 경로(1·3) 발견도 questReveal 배선(기록만 되고 서술에 안 실리던 공백) — arch/58 §6. **C 에드릭 speechStyle 어구 예시** "(삼할/두 자루/엿새 전 등)" — 30일 352턴 중 47% "삼할" 축자 → 구성 요소 지시로 교체 + `SPEECHSTYLE_LITERAL_EXAMPLE` 감사 규칙. 스펙 +9, 2,524 passed, 프로브 3종 PASS. 재확인(미수정): 잡담 화이트리스트가 질문형 TALK 45% 에 화제 주입, 프리셋 "약초 냄새가 밴 손" 엔진 리터럴 앵커(45%), 빈손 가드가 "보유"만 보고 "이번 질문으로 공개 가능"을 못 봄 | ✅ 완료 |
| **turns.service 3차 재비대 대응 — LOCATION 턴 서비스 분리 (2026-09-02)** | [arch/112] 래칫 재발화(파일 7,975/7,900·함수 2,047/2,000, §19 마진 63줄이 이틀에 소진). 계측: LOCATION 클러스터 36메서드 6,998줄(파일의 88%)·장수 변수 34개·미사용 주입 10·사문 메서드 1. Phase 0 사문 제거 → Phase 1 `LocationTurnService` 통째 이관(TurnsService 587줄 라우터, runInTurnContext 래퍼 불변, 메서드 45개 원문 기계 대조) → Phase 2 `LocationQuestService`·`LocationResultService` 분리 + 오케스트레이터 3블록 추출(pendingQuestHint·quest_forward/빈손·이벤트 조립) → 함수 1,620. 래칫 상한 불변(§18 원칙), 레포 lint 경고 0, 2,524 passed·스냅샷 불변·스모크 PASS. 함정: BSD sed `\+` 무효·eslint 자동수정이 params any 화로 `(result.ui as any)` 단언 11줄 제거(타입 전용)·finalizeRunEnding 은 정산 헬퍼 의존 때문에 turn-shared 대신 LocationTurn public. Phase 3 계획(§9): P3-A UI 타입화 → P3-C 정적 배선 디텍터 → P3-B LocationTurnContext → 테스트베드 스파이크. **P3-A 완료(2026-09-02, server a4e124a)**: UIBundle +24필드·생산 측 캐스트 0·소비 측 10곳 타입 읽기, 동작 0 변경. 드러난 것 4건(MOVE 스탬프 originalInput 부재 등)은 §10.1 기록. **P3-C 완료**: `scripts/selfcheck/wiring.py` W족 4항목(ui 부착↔타입·캐스트 계측·nano 상태형 필드↔검증·currentRoute 커밋 오용) — 첫 실행에서 W3 가 **결심 선택지 게이트 결함**(힌트 루트 런 영구 커밋 불가, scene-shell·location-quest)을 잡아 isArcCommitted 정본으로 수정, 스펙 3. 검사기 오탐 4종 등재 §10.2. **급행 2건(09-03)**: 힌트 루트 배신 카운트 제외(`switchRoute`→isArcCommitted) · 빈손 가드 STEER 모드(보유하지만 질문이 안 닿는 NPC 는 주제 이름만 비추고 "X에게 {topic}에 대해 묻는다" 선택지 보장 → 다음 턴 키워드 매칭 공개) §10.3 | ✅ 완료 |
| **보안 감사 + 후속 수정 14건 (2026-09-07)** | [arch/115] `/cso` daily 감사(4레포) HIGH 6·MED 10 → 소유자 지시로 전항목 수정. **서버(814e786)**: H1 캠페인 IDOR(`getCampaign(id,userId)` 선행)·H2 파티 턴 상세 cross-run IDOR·H3 투표 파티 검증+원자 조건부 UPDATE(더블캐스트 race)·H4 deps(drizzle 0.45.2·nest 11.2.3·sharp 0.35.4, stale package-lock 삭제)·M1 includeDebug `llmPrompt` 어드민 토큰 한정·M2 `trust proxy loopback`(cloudflared 뒤 per-IP 제한이 전역 버킷 하나로 붕괴하던 결함)+기본 127.0.0.1 바인딩(`HOST`)·M3 `?token=` 쿼리 인증 `@AllowQueryToken` SSE 2곳 한정·M8 settings/llm PATCH Zod strict allowlist·M9 파티원 닉네임 sanitize·`POST /v1/auth/logout` 신설. **클라(8b836ac·7ab6247)**: JWT localStorage 제거→httpOnly 쿠키 세션·CSP/HSTS/XFO 헤더·next 16.3.4. **시크릿·백업(docs c664f72)**: 테스터 비밀번호 리터럴 29곳→`PLAYTEST_PASSWORD` env·ADMIN_TOKEN 회전·`.env.bak` 삭제·`scripts/backup-db.sh`(launchd 04:30 pg_dump, 14일 보존·iCloud 사본)·docker-compose `POSTGRES_PASSWORD` env 참조. 후속: 플레이테스트 V12 게이트가 llmPrompt 를 못 받아 보류되던 것을 `x-admin-token` 첨부로 복구(be4ce9f). **소유자 잔여 3건**(TODOS.md): DB 비밀번호 회전·Slack 웹훅 재발급·구 OpenRouter 키 revoke | ✅ 완료 (잔여 3건 소유자) |
| **거대 파일 4개 조각내기 T1·T2·T3 (2026-09-08)** | [arch/114] 재작성 대신 유지보수(§0 — "실측" 주석 540·인용 런 41·버그 17·불변식 58 이 코드에만 있다). **T1** llm-worker 6,765→3,965(`processTurnInner` 1,955→60, 단계 메서드 10 + `llm/worker/` 로 후처리 헬퍼 군 2,290·CAS 66·core 756 이관, byte 대조) **T2** 프롬프트 블록 레지스트리 90 항목+배타 3쌍+픽스처 스펙(사후 분류, 빌더 무변경) **T3** `SoftStateView` 로 불변식 2 컴파일 강제(CAS 11곳 수정 0 통과, `@ts-expect-error` 9줄 고정 — 대괄호 접근은 `noImplicitAny:false` 라 미보증). jest 2,592→2,614·스냅샷 17 불변·스모크 PASS. server `3656c03` | ✅ 완료 |
| **품질 사이클 1~3회차 (2026-09-08)** | `/quality-cycle` 3회(graymar·chatty / star_sand·coercer / graymar·weirdo, 10턴) — 게이트 15/15, 정독에서 9건(server `96e9d5a`): 이동 전환 지시에 떠나는 주체(플레이어) 명시(NPC 퇴장으로 그려짐)·거점 복귀 셸(HUB+MOVE)엔 [참고 선택지] 미주입(방금 떠난 곳으로 "향하는 갈림길" 낭독, 셸 192건 중 35%)·[무명 화자 프레이밍] 완성문 예시 제거(30일 35턴 축자, arch/105 P0-3 동형)·`CROWD_CUE_RE` 복수·낯선 목소리 신호(군중 반말이 직전 화자에 귀속→어체 위반)·`stripModelJunkCore`(j) 마침표 뒤 코드형 토큰·**이벤트 discoverableFact 경로 1 FREE 대화 게이트**(기행·잡담 턴이 자동 SUCCESS 라서 단서 100% 낙하 — 불변식 27 보강)·FREE 턴 fact 본문 주입(`resolveOutcome` 공백으로 기록≠서술, 30일 ≈80턴)·활동 이벤트 discoverableFact 는 스케줄 NPC 보유분만(knownBy 밖 S4→S5 관찰 공개 7건)·includeDebug llmPrompt 어드민 토큰 한정(M1 후속). 소유자 판단 대기: 단계 앞선 fact 조기 공개 23% 에 stage 게이트를 둘지. 보고서 `playtest-reports/qc_20260908_summary.md`(gitignore — 추적하려면 `git add -f`) | ✅ 완료 (게이트 판단 1건 대기) |
| **품질 사이클 4~6회차 + 문서 정합 점검 (2026-09-08~09)** | `/quality-cycle` 3회(star_sand·chatty / graymar·devotee / star_sand·sneaky_liar) — 게이트 15/15, 6건(server `a74f9fa`·docs `f5b114a`): ① 시스템 프롬프트 예시의 콘텐츠 실명 "로넨"→역할 명사(불변식 45 — 별빛모래 런에 주인공 이름으로 샘) ② 저작 접근 선택지 9종(scene-shell)+콘텐츠 `encounter_talk` 에 `riskLevel: 1` + `choice-challenge.core` ALWAYS_CHECK 면제 → 판돈 없으면 FREE(불변식 40 보강). **1차안(affordance→TALK)은 철회** — affordance 는 이벤트 매칭 축이라 저작 funnel(30일 72%)이 끊긴다 ③ `tone-hint.core` `resolveSkipped` → FREE 사회 행동은 calm(불변식 53 보강) ④ nano `[첫 문장]` 인물 감정 클리셰 가드 `OPENING_EMOTION_CLICHE`(30일 20%·복제 69%; "공기가 차갑게" 오탐 회귀 스펙) ⑤ `normalizePeekVerbs` 훔쳐보다→엿보다(구 STEAL 매칭이 골드·전리품 실지급). **09-09 문서 정합 점검**에서 발견한 코드 이격 1건: nano `[첫 문장]`(환경 감각)이 arch/110 `[첫 문장 지시]`(잠금 턴 상대 반응 직행·ACTION_RESULT/NPC_MOTION "환경으로 열지 말 것")와 14일 714턴 동시 주입, 532턴 모순 → `nanoOpeningAllowedForOpener`(진입·HUB·SENSE/ENVIRONMENT 턴에만 nano opening) 게이트, 스냅샷 8건 갱신(diff = [첫 문장] 줄 제거뿐) — arch/110 §6 | ✅ 완료 |
...
| 25_llm_model_evaluation.md | ✅ 정본 | LLM 모델 평가 (v1~v5 통합) + 운영 모델 변천. **부록 E(2026-08-13)가 현 구성 정본** — 교차 Luna 5:5·ModelRun 제거·fallback 404 해소·Gemini 보류·검증도구 결함 6종. **부록 F(2026-08-14)** — Gemini 3.x 후보 평가(3.6-flash 탈락·3.5-flash-lite 보류) + flex 티어/Vertex temperature 함정 + 도구 결함 7·8. **부록 G(2026-08-27)** — GLM 5.3 Flash 보류(라벨 66.7~76.3% = DeepSeek 탈락 구간) + 프로바이더가 속도·품질을 가른 실측 + 추론 강제 모델 대응 |
...
| 58_fact_reveal_unification.md | ✅ 구현됨 | 단서 기록·서술 단일화 + §6 nano `[정보 전달]` 지시 questReveal 정규화·이벤트 경로 questReveal 배선 (2026-09-02) |
...
| 77_god_method_refactoring.md | ✅ 구현됨 | God method 리팩토링 (§18 분할 2026-08-07 · §19 재비대 대응 2026-08-31 · 3차는 arch/112) |
...
| 102_location_image_content_pool.md | ✅ 구현됨 | 이미지 정본 단일화 — location_images.json 콘텐츠 승격 + 초상화 npcs.json portraitUrl 외부화 + 마커 npcId 전환 + 장소 URL 서버 이관 + 클라 레거시 리졸버 삭제·hubImage 승격(2026-08-31 잔여 마감) |
| 103_arc_endgame_stages.md | ✅ 구현됨 | 아크 종반부 3막 — 사문이던 arc 스테이지 이벤트(getArcEvents 소비 0곳) 배선 + 결말 선택지(arc_finale) + S5+5 안전망 HUB 확장 + 커밋 후 안내 (버그 3c501bf7) + **§6 무커밋 진행·안내 턴 즉시 완수 수정(2026-09-02 — committedAt 게이트)** |
| 104_npc_dialogue_gate_rework.md | ✅ 구현됨 | NPC 대화 게이트 재설계 — 잡담 주입 화이트리스트 전환(small-talk-gate.core)·소진 폴백 제거 + 대화 잠금 3상태/streak 상한/BACKGROUND 제외(conversation-lock.core)·자기강화 루프 차단. 불변식 23·26·44·50 개정 (잔여: P2 5종 — 되묻기 억제는 2026-09-01 server 4937281 완료, daily_topics 축약은 2026-08-18 완료) |
...
| 107_legal_compliance.md | ⚠️ 부분 구현 | 법적 고지·규제 준수(상품화 P0-2) — 이용약관·개인정보처리방침 신설(`/terms`·`/privacy`, 표시사항 정본 `operator.ts`)·가입 동의 게이트 + **비공개 테스트 초대 코드 게이트**(`invite.core`/`invite.service`, fail-closed·원자적 클레임, `INVITE_CODE_REQUIRED`). 국외 이전(OpenRouter·Vercel·Cloudflare) 고지가 핵심. §7 유사 서비스 실측(GPTRPG·프론티아·크랙·제타 — 전원 등급분류 미표시), §9 TODO. §5 회원 탈퇴는 2026-08-31 구현(`POST /v1/auth/withdraw` 비밀번호 재확인 + 어드민 삭제와 cascade 정본 공유 `user-deletion.service` + 설정 모달 2단 확인 — PG 연동 시 유상 원장 분리 보관 재설계 필요). **미해결: 게임제작업 등록(제25조, 미등록 = 2년/2천만원)·게임물 등급분류(제21조)·`operator.ts` contactEmail 공란** |
| 109_npc_relationship_engine.md | ✅ 구현됨 | NPC 관계 엔진 R1~R3 — kind(FRIEND/CONFIDANT/ROMANCE/RIVAL/GRUDGE) 전이 정본 `relationship-kind.core`·tier 파생·동행 1명(`companion.core`)·양다리 들킴 결정론 3경로(`affair.core`)·nano relationSignal 은 감지만 (불변식 56~58) |
| 110_narrative_opening_variance.md | ✅ 구현됨 | 서술 개시 다양화 — 환경·시간대 개시 60%·대사 개시 0% 실측 → 잠금 턴 장면 리셋 금지 + 4유형 결정론 로테이션(`opener-directive.core`) + 팔레트 턴별 샘플링 + 사교 발화 분량 변주. **§6(2026-09-09)** nano `[첫 문장]` 모순 게이트 |
| 111_incident_rebalance.md | ✅ 구현됨 | Incident 재기준화 — 30일 554건 해소율 4.3%(CONTAINED 24·ESCALATED 0·EXPIRED 0) → 임계 재기준화 + 해소 시그널(A안) + 방치 드리프트(B안, 2026-08-31 소유자 채택), 40턴 롱런 검증 |
| 115_security_audit_20260907.md | ✅ 구현됨 (소유자 잔여 3) | 보안 감사 후속 — IDOR 3·trust proxy·SSE 전용 query token·설정 allowlist·닉네임 sanitize·쿠키 세션·시크릿 env 이관·DB 백업. 잔여: DB 비밀번호 회전·Slack 웹훅·구 API 키 revoke (TODOS.md) |
| 108_natural_equipment_acquisition.md | ✅ 구현됨 | 자연스러운 장비 획득 3경로 — 30일 실측 LOCATION 61%(TALK·INVESTIGATE·PERSUADE)가 장비 경로 배제. A 관계 선물(trust 임계 대화 턴 1회)·B 단서 연계 은닉(발견 fact×장소 조사 확정)·C 개인 거래(NPC personalTrade, bribeOpportunity 미러 nano 노출). 발동은 서버 결정론·런당 1회·diff.equipmentAdded 서술 정합(arch/40)·콘텐츠 저작(불변식 45). **대화(HELP) 랜덤 드랍 제거**(EQUIPMENT_DROP_ACTIONS=STEAL/THREATEN/FIGHT/SEARCH, 골드 사례 유지). natural-acquisition.core 스펙 23 + A·B 실런 실증 + audit_content 모양 검사 |
| 113_solar_dialogue_profile.md | ✅ 병합·전턴 운영(개발 기간) | Solar Pro 4 전용 모델 프로파일 — 라벨 재강조 접두/접미(V1 실증 57→80%) + 화자 연속 귀속(둘째 무라벨 대사→직전 화자, 군중 신호 가드) + v2(대명사·호칭·화폐 재강조, 다중 인용 분리) + 로어북 미발견 fact 세부 게이트. 실런: 라벨 70→82%·어체 16.7→8.1%·무라벨 3→0. 전문 평가 §5 |
| 112_location_turn_service_split.md | ✅ Phase 0~2 + P3-A·P3-C 완료 | turns.service 3차 재비대(7,975/7,900·함수 2,047/2,000) 대응 — LOCATION 파이프라인을 location-turn(5,171)으로 이관 + location-quest(1,108)·location-result(1,289) 분리, 오케스트레이터 1,620줄. 래칫 상한 불변·lint 경고 0·동작 보존 기계 대조. Phase 3(LocationTurnContext·UI 부착 타입화·배선 정합 스펙)은 트리거 명시 후속 |
| 114_llm_worker_stage_pipeline.md | ✅ T1·T2 Phase A·T3 완료 | 거대 파일 조각내기 — llm-worker 6,765→3,965(processTurnInner 1,955→60, 단계 10 + `llm/worker/` core·CAS·후처리 이관, byte 대조) · 프롬프트 블록 레지스트리 90 항목 + 배타 3쌍 + 픽스처 스펙(사후 분류, 빌더 무변경) · `SoftStateView` 로 불변식 2 컴파일 강제(CAS 11곳 수정 0 통과, 대괄호 접근은 noImplicitAny=false 라 미보증). 재작성 대신 유지보수 결정 근거 §0 |
