...
| **NPC 이름 공개 무결성** | A~E + B(pendingIntroduction, 조용한 공개 제거) + **연출 성공률 3층 방어**: 성향/실패이력 기반 경로 분기(자기소개→제3자 호명/단서) + introAttempts 추적 + IntroFallback(2회 실패 시 서버 호명 문장으로 결정론 마감). 핍 T4~T11 실증, 회귀 23건 — architecture/64 | ✅ 완료 |
