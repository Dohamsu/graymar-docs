...
├── architecture/        ← 통합 아키텍처 문서 (48 md + INDEX.md 색인, 실무 참조, archive 4 md)
...
15. **NPC 이름 비공개→공개** — FRIENDLY·FEARFUL 1회 / CAUTIOUS 2회 / CALCULATING·HOSTILE 3회 소개. 2턴 분리: 소개 턴엔 별칭 유지(LLM이 이름 밝힘 연출), 다음 턴부터 실명 — 모든 소개 경로는 `introducedAtTurn` 필수. LLM이 이름을 언급하지 않으면 IntroRollback으로 취소 + pendingIntroduction 이월. 워커(AppearanceIntro)는 공개를 직접 찍지 않고 pending만 마킹 — 승격은 해당 NPC가 장면에 등장하는 턴에 소개 연출 지시와 함께 (조용한 공개 금지) — architecture/64.
...
44. **사교 발화 fact 게이트** — 순수 사교 발화(dialogueAct: GREETING/WELLBEING/THANKS/FAREWELL, `common/dialogue-act.ts`) 턴은 NPC fact 공개 경로·인계/보류 힌트를 타지 않는다 (잡담 모드로 전환). 질문 턴은 비주제 fallback fact 공개 금지 — 물은 것과 무관한 단서로 답하지 않는다. BRIBE/THREATEN은 면제.
...
| **NPC 이름 공개 무결성** | A~E: 롤백-재소개 상쇄 차단 + 표시명 turnNo 일괄(2턴 분리) + 소개 힌트 실명 콘텐츠 직접 + injected introducedAtTurn + 별칭 접두 중복 2중 정리 + **B: pendingIntroduction — 조용한 공개 제거, 모든 공개가 연출 지시→검증 파이프라인 통과**. 핍 케이스 실증·실런 검증, 회귀 15건 — architecture/64 | ✅ 완료 |
...
### architecture/ — 통합 아키텍처 (48 md + INDEX)
