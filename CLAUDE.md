...
├── architecture/        ← 통합 아키텍처 문서 (82 md + INDEX.md 색인, 실무 참조, archive 4 md)
...
### architecture/ — 통합 아키텍처 (82 md + INDEX)
...
| 25_llm_model_evaluation.md | 📎 참고 | LLM 모델 평가 (v1+v2+v3 통합) + 운영 모델 변천 부록. **현 운영: Gemma 4 31B dense 메인 (allowlist ModelRun·Friendli) + DeepSeek V4 Flash 짝수 턴 교차 + GPT-4.1 Mini fallback (부록 D-8)** |
...
| 75_autonomous_pack_design.md | ✅ 구현·배포 (P7 후속 대기, P8 후속=83 완료) | 자율 서사 팩 "진상 선확정 디렉터 모드" — 문답 14결정 고정(L3·코어 외 NPC 전부 생성·장소 저작·진상 선확정 Plot Seed·3막+게이지 종결·매 런 캐스팅·모티프 풀 조합·비동기 선계산·자율 팩 재진입 예외·주민화·규명율 엔딩) + 해석 심(P0 스파이크 관문) + P0~P8 단계. **P0~P6+P8 구현·배포 (2026-07-16, karnholt_v1 AUTONOMOUS 팩)** — PlotSeedGenerator(진상 선확정+검증/폴백) + PlotDirector(비트 선계산·워커 CAS)+동기 채택(beat-gravity 불변식 47) + 동적 NPC + 규명율 엔딩 + 킬스위치. P8 계측 후속(디렉터 존재감)은 **83에서 안 A+C로 구현 완료**(채택 2.0/12턴) (2026-07-15 설계 → 2026-07-16 배포) |
...
| 81_day_night_system.md | ✅ 구현됨 (2차 포함) | 밤낮 시스템 재설계 — 행동 가중 timeCost + 전환 서술 주입(recentPhaseTransition) + 4상 UI 승격 + **이중 시간계 통합**(deriveTimePhaseFromV2, v1 advanceTime 토글 폐지) + **2차(2026-07-25): 시간은 이동·시간 소요 행동에서만** — 대화 계열 0-cost·이동 3경로 시계 배선 복구(advanceClockForTravel, 죽은 MOVE 비용 실측)·transPhrase anchor 제거. 잔여: 시간대별 특이 이벤트(콘텐츠) (2026-07-20/25) |
...
| 92_hub_base_location_collision.md | ✅ 구현됨 | 거점(HUB) ↔ 거점 장소 정체성 충돌 — 같은 고유명이 추상 상태(HUB, `currentLocationId=null`)와 실제 LOCATION 에 동시 부착돼 "꿈잠 여관에서 꿈잠 여관으로 이동"이 성립하던 결함. A안(이름 분리) 채택 — 4팩 `hub.name`/`returnLabel` 추상형("<지역> 거점") + `regionSummary`·L0 `hub_system` 프레이밍 교체(라벨만 고치면 의미 층에서 재생산) + 자기 자신 이동 선택지 제거 + HUB 복귀 턴 도착 디렉티브 분기(없는 장소 환경 묘사 강제 → 세계관 위반 실측). D9(거점 진입 4턴)는 B안 백로그. **§8~9 스핀오프**: V9 반복 센서 정밀화(실패 17%→6%, 92%가 센서 아티팩트 — 슬라이딩 윈도우 다중 계수·고정폭 절단 토큰화, `scripts/repetition_core.py` 정본 분리) + `_npc_alias_pool`·`events_v2.json` graymar 경로 하드코딩 발견(비-graymar 런이 남의 팩으로 판정). **§10**: 부재 NPC 화자 승격 근본 수정 — 별칭 부분 매칭(`'수상한 곳'`이 `'수상한 관리인'`에 매칭)으로 다른 장소 NPC 가 화자로 승격 (2026-07-26) |
| 93_location_backdrop.md | ✅ 구현됨 | 장소 배경 지속화 — 대화 중 장면 이미지 공백(중앙값 3턴·p90 6턴·최장 23턴, 532구간 실측)을 배경 레이어로 메움. 헤더 밴드는 기각(모바일 뷰포트 25% 잠식 = arch/86 로 확보한 서술 영역 반납), orphan `LocationImage.tsx` 는 크로스페이드만 이식 후 삭제. 장소·시간대 변경 시에만 교체, 신규 에셋 0. **§7 후속**: 장소 라벨 정본화 (2026-07-27) |
| 94_mobile_scroll_viewport.md | ✅ 구현됨 | 모바일 스크롤·뷰포트 정합 — follow 모델(제스처 우선·프로그램 스크롤 창 구분) + overscroll-contain 전면 + 타이틀/로그인 스크롤 구조 + 모달 2패턴 + `MOBILE_HEADER_OFFSET` 단일 정본 + safe-area 커버리지. 규약: 스크롤러에 `min-h-full` 금지(바깥 높이 제약/안쪽 최소 높이), 중앙형 모달은 패널 `m-auto` 필수(flex 중앙정렬은 위쪽 오버플로 도달 불가). 헤드리스는 safe-area inset 0 보고 → 계산식 정합까지만 검증 (2026-07-27) |
...
### guides/ — 코드 구현 지침 (10 md)

| 파일 | 내용 |
|------|------|
| 01_server_module_map.md | 서버 전체 서비스 맵 (107 services, 47 타입 파일) |
