...
# [M3 감사 2026-08-12 — 체크리스트 C6] 경계 부재 알려진 한계.
#   EASTERN/CURRENCY 와 달리 이 목록은 (?<![가-힣]) 경계가 없다. CLAUDE.md 의
#   "word boundary 없이 키워드 매칭하지 않는다" 규칙과 어긋난다.
#   다만 실측 오탐률은 낮다 — 782턴 원시 매칭 4건(0.5%)이고 그중
#   ① 대사 내부("…생각한다면") ② 감각 지각("공기를 느낀다") 둘 다
#   3단 심층 검사(대사 내부/외부 판정)가 걸러낸다.
#   경계를 붙이면 활용형("느꼈다는")을 놓치므로, **경계 추가 대신 심층 검사
#   의존**이 현 설계다. 심층 검사를 우회하는 신규 소비처가 생기면 재검토.
META_NARR_FORBID = [
...
# 조사·허용 합성어 뒤에도 오른쪽 한글 경계를 확인해 임의 합성어 오탐을 막는다.
KOREAN_TERM_END = r'(?:으로|에서|에게|부터|까지|처럼|만큼|조차|[은는이가을를에의로도만과와])?(?![가-힣])'
...
EASTERN_FORBID = {
    ...
    '고추장': r'(?<![가-힣])고추장(?![가-힣])',
    '소주':   r'(?<![가-힣])소주(?![가-힣])',
    '젓가락': rf'(?<![가-힣])젓가락{KOREAN_TERM_END}',  # 조사 결합(젓가락을)도 감지
    # [M3 감사 2026-08-12 — 체크리스트 C7] '막사' 제거.
    #   시스템 프롬프트(P0-K)와 CLAUDE.md 가 금지하는 동양 요소는
    #   막걸리/김치/온돌 뿐이다. '막사'(幕舍)는 서양 중세에서도 쓰는 일반
    #   군사 용어(barracks/tent)라 금지 대상이 아닌데 사전에 들어 있어
    #   매 런 회색 판정을 만들어 왔다 (실측: ae204f16 런의 유일한 회색 1건).
    #   검사기가 설계보다 넓게 금지하면 노이즈만 늘고 진짜 위반이 묻힌다.
}
CURRENCY_FORBID = {
    '은화':   rf'(?<![가-힣])은화{KOREAN_TERM_END}',
    '은전':   rf'(?<![가-힣])은전{KOREAN_TERM_END}',
    '금화':   rf'(?<![가-힣])금화{KOREAN_TERM_END}',
    '동전':   rf'(?<![가-힣])동전(?:주머니)?{KOREAN_TERM_END}',
    '닢':     rf'(?<![가-힣])닢(?:짜리)?{KOREAN_TERM_END}',
}
