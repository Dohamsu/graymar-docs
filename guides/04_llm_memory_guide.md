...
| L1+ | midSummary | (생성) | 6턴 초과 시 초기 턴 200자 요약 |
| L2 | nodeFacts | node_memories | 현재 노드 사실 |
| L3 | locationSessionTurns | turns | 현재 방문 대화 (최대 6턴, MidSummary 적용) |
...
`[World State]` → `[Narrative Milestones]` → `[Story Summary]` → `[NPC Relationships]` → `[Incident Chronicle]` → `[Extracted Facts]`
...
| STRUCTURED_MEMORY | 500 | [이야기 요약]+[NPC 관계]+[사건 일지]+[서사 이정표]+[기억된 사실] |
| BUFFER | 300 | 기타 블록 (장비, 성향, NPC 목록 등) |
| **TOTAL** | **2500** | |
...
## Mid Summary (6턴 제한 + 중간 요약)

`server/src/llm/mid-summary.service.ts` — 설계문서 18

```
locationSessionTurns > 6턴
  → 초기 턴들을 200자 요약으로 압축 (midSummary)
  → 최근 6턴만 locationSessionTurns에 유지
