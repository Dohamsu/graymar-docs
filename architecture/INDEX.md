...
- `01_world_narrative.md` — 그레이마르 왕국의 정치 음모 배경과 3 루트(부패 폭로/혼란 이익/근위 동맹) 서사 골격. 모든 서술의 톤과 사건 시드의 뿌리.
- `09_npc_politics.md` — 5축 감정(trust/fear/respect/affection/hostility)과 NPC 소개 조건(FRIENDLY 1회/CAUTIOUS 2회/HOSTILE 3회), posture 전환 규칙. Leverage/거래는 부분 구현.
...
- `06_graymar_content.md` — 프리셋 6종, NPC 43명, 장소 7개, Incident 13건 등 graymar_v1 콘텐츠 스키마와 시드 데이터 구조.

### 2. 게임 엔진

- `02_combat_system.md` — 정본 전투 공식(`hitRoll → varianceRoll → critRoll`, floor 적용), BattleState 저장, 거리/각도 처리. `specs/combat_system.md`와 정합.
...
- `07_game_progression.md` — HUB 순환 → LOCATION 탐험 → 엔딩의 흐름. HUB 모드 도입 이후 업데이트 필요.
- `14_user_driven_code_bridge.md` — 플레이어 입력 → IntentV3Builder → IncidentRouter → WorldDelta → PlayerThread → Notification의 유저 드리븐 브릿지.
- `21_living_world_redesign.md` — Living World v2 전면 재설계(LocationDynamicState/WorldFact/NpcSchedule/NpcAgenda/SituationGenerator/ConsequenceProcessor/PlayerGoal).

### 5. LLM·서술 파이프라인

- `05_llm_narrative.md` — LLM 파이프라인 개요(L0~L4 컨텍스트, Token Budget 2500, LLM is narrative-only 불변). 모든 서술 문서의 진입점.
...
- `15_notification_system.md` — Notification 아키텍처(설계/UI/브릿지 통합). 기존 `15/16/17` 3개 문서가 하나로 통합됨. 클라 실제 연결은 `guides/02_client_component_map.md`.
- `23_dialogue_ui_redesign.md` — 대화 UI 고도화(메신저 형태, NPC 카드 연출, 홑따옴표 강조).
...
| 작업 유형                | 먼저 볼 문서                                                                 |
| ----------------------- | -------------------------------------------------------------------------- |
| 새 서버 서비스 추가      | `04_server_architecture.md` → `guides/01_server_module_map.md`             |
| LLM 프롬프트 수정        | `05_llm_narrative.md` → `26_narrative_pipeline_v2.md` → `guides/04_llm_memory_guide.md` |
| 스트리밍/SSE 관련        | `35_llm_streaming.md` → `guides/04_llm_memory_guide.md`                    |
| 이벤트/NPC 매칭          | `34_player_first_event_engine.md` → `03_hub_engine.md` → `guides/03_hub_engine_guide.md` |
...
| 파티 기능                | `24_multiplayer_party_system.md` → server `party/` 모듈                    |
