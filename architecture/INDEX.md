...
- [[architecture/81_day_night_system|day night system]] — ✅ 구현·배포. 밤낮 이중 시간계 근본 해소: ①행동 가중 timeCost(사교 0/이동·휴식 2/기타 1) ②전환 서술 주입(recentPhaseTransition → 전환 턴만 [시간대 전환] 디렉티브) ③4상 UI 승격(phaseV2·day, 황혼 오표기 해소) ④**이중 시간계 통합**(deriveTimePhaseFromV2 — v1 advanceTime 토글 폐지, timePhase=phaseV2 미러, 전투 경로 불일치 해소, 불변식 49). 실측 전환 5회→1회. **3차(2026-08-13)**: 대화 지연 틱 6→2 + 발효 상한 2 신설(적립분이 평균 3.90 잠긴 채 런 종료 · 이동 경로가 `% N` 으로 초과분 소실하던 것도 해소). 실효 시계 0.39→0.43 tick/턴 — 예상 미달, 누적 재측정 필요. 잔여: 시간대별 특이 이벤트(콘텐츠).
...
- [[architecture/21_living_world_redesign|living world redesign]] — Living World v2 전면 재설계(LocationDynamicState/WorldFact/NpcSchedule/NpcAgenda/SituationGenerator/ConsequenceProcessor/PlayerGoal). **Part 11 세계 자율 진행 복구**(2026-08-13) — "세계가 고요하다" 피드백의 원인이 사문 배선 3건이었다: `longTermAgenda`(구조체) vs 저작된 `agenda`(문자열) · 저작 15 스테이지가 전부 `day` 게이트인데 91% 런이 day 1 종료 · `conditionApply` 키 불일치. 해소 후 worldFact `NPC_ACTION` 이 30일 10건 → 한 런 8건. 잔여: `WORLD_CHANGE`/`RELATIONSHIP` 쓰기 지점 0곳.
...
- [[architecture/79_prompt_token_optimization|prompt token optimization]] — ✅ P3~P4 완료. 측정 기반 프롬프트 예산: P1 회고 1,556턴(hard 규칙 15k 강건/soft 문체 11k 절벽) → 예산 10k → 실행 4파트(재시도 스킵 16.5%→0 · 시스템 -62% · NPC 클러스터 압축 · 총량 백스톱 16,000자). 최종 avg 7,495tok(-31%)·절벽 턴 0%·게이트 7런 10/10·회귀 0. 대화 턴 대명사 기저는 크기 무관 확정(교란 재해석 §9.1) — arch/78 백로그 이관 (2026-07-19). **§10 2·3차 재압축(2026-07-27~28)**: 3주 재비대(발동 35%·[NPC 일상] 상시 삭제) → 압축 13종+백스톱 순서 교체+상한 16,500자+V12 재비대 게이트 — 발동 8%·D 삭제 0건·시스템 5,542자. **§13 백스톱 20,000자 상향(2026-09-01, 감시 조건부)**: 30일 실측 밴드별 어체 위반율 평탄 근거 — 텔레메트리(llm_token_stats)+정본 모니터(prompt_budget_monitor.py, Luna 밴드 +5%p = 롤백 신호)+V12 원장 v2.
- [[architecture/95_prompt_split_analysis|prompt split analysis]] — 📜 종결(폐기). 프롬프트 이분할(서술·대사계) 재검토: 원형은 스트리밍 상호 배타로 재기각, 역전 설계(대사 선행 생성→주입)는 파일럿 실증(채택 94~100%·어미 93~96%·응답률 보완) 후 확장 배터리에서 재시도 증폭 실측 → 소유자 결정 폐기(archive/spike-dialogue-precommit). 부산물: DeepSeek 교차 어미 열세 실측 → 교차 5:5→3:7 (2026-07-28).
