...
- [[architecture/78_narrative_opener_pronoun_cycle|narrative opener pronoun cycle]] — 서술 개시어·대명사 편중 억제: D5 계측 센서(대명사 개시어율·지칭 명사구 분류) + 개시어 임계 3→2 + 대명사 화이트리스트 12종 1키 합산. 실측 20.3%→16.2%(상대 -20.2% — 잔여는 soft 지시 천장). 즉흥 별칭 가설은 실측 기각(전부 콘텐츠 별칭·축약형). ✅ 수용 종결, 2차 처방은 백로그 (2026-07-18).
- [[architecture/79_prompt_token_optimization|prompt token optimization]] — 📐 계획. 측정 기반 스테이지별 프롬프트 예산: P0 실측(메인 서술 avg 10,856tok = 명목 예산 4.3배·재시도 16.5%·intent 3.9k/출력 37tok) + P1 준수율×토큰 상관(기존 로그 회고) + P2 곡선 무릎에서 예산 고정 + P3 최적화(enforceTotal 전 블록 확장·intent 예시 A/B·캐싱 arch/11 소생) + P4 게이트 (2026-07-18).
