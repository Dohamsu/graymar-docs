...
### 10.3 급행 2건 (2026-09-03, 소유자 결정)

1. **힌트 루트 배신 카운트** — `switchRoute` 의 배신 판정을 `isArcCommitted` 로. 힌트만 맞은 런의 첫 결심은 배신이 아니다.
   스펙 3(힌트 route-only 전환 배신 0 · committedAt/레거시 전환 +1 · route 없음 0).
2. **빈손 대화 가드 신호 재정의** (`empty-handed-hint.core` STEER 모드) — 30턴 분석 §3-④ 갭. "보유 fact 0" 만 보던 가드가
   마이렐처럼 **보유하지만 질문이 키워드에 안 닿는** 경우(실측에선 이쪽이 더 흔함)를 놓쳤다. 보유 fact 가 있고 이번 턴 공개가
   없으면 `mode: 'STEER'` + `steerTopic`(다음 공개 후보 fact 의 `facts.json topic`) — 프롬프트는 "지금 질문엔 모른다 + 주제
   이름만 스치듯"(내용 공개 금지, 불변식 27), 워커는 "X에게 {topic}에 대해 묻는다" 선택지를 보장한다. 이 라벨은 fact 키워드에
   매칭돼 다음 턴 공개로 닫힌다(캐묻기 라벨과 같은 원리). `ensureNpcTargetChoiceCore` 에 `topic` 옵션(중복 판정은 라벨 포함).
   실런 프로브(run ef89345d, 경비대): T4 첫 접촉 무동작 → T5·T6 STEER(topic 내부자 의혹) · 프롬프트 블록 주입 · 선택지
   "야간 경비 책임자에게 내부자 의혹에 대해 묻는다" 주입 · 마이렐 발화 "내 소관은 밤의 치안이지… 내부자들 사이에서 도는
   묘한 의혹들이 더 골칫거리라는 것 정도만" — 주제 이름만, 내용 없음. 스펙 +7(코어 3·선택지 1·arc 3).
