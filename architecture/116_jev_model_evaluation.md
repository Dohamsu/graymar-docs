...
재현 스크립트 `scripts/jev-next-candidates-eval.py`로 챌린지 `FREE/CHECK` 24건, 사전 필터된 이벤트 후보 선택 16건, 선택지 라벨→affordance 30건을 Jev와 `openai/gpt-4.1-nano`에 동일하게 주었다. 부정문·복합 행동·소문/실제 사건 구분·prompt injection을 경계 사례로 포함했다. 같은 70건을 2회 반복했고 Jev의 선택은 두 실행 모두 `70/70` 일치했다.

| 후보 | 표본 | Jev 정확도 | nano 정확도 | Jev p50 / p95 | confidence gate |
|---|---:|---:|---:|---:|---:|
| 챌린지 `FREE/CHECK` | 24 | **100%** | 79.17% | **254 / 354ms** | `≥0.9`: 70.83% 커버리지, 100% 정확도 |
| 이벤트 후보 선택 | 16 | **100%** | 93.75% | **260 / 296ms** | `≥0.85`: 100% 커버리지, 100% 정확도 |
| 선택지 affordance | 30 | **100%** | 63.33% | **260 / 366ms** | `≥0.9`: 80% 커버리지, 100% 정확도 |

2회차의 Jev 총비용은 챌린지 `$0.000412`, 이벤트 `$0.000348`, affordance `$0.000828`이었다. 같은 표본의 nano는 각각 `$0.000472`, `$0.000449`, `$0.001078`이었다. 원시 보고서는 `playtest-reports/jev_next_candidates_20260921_143652.json`, `playtest-reports/jev_next_candidates_20260921_143736.json`이다.

판단은 **이벤트 선택 → affordance 검증 → 챌린지 분류** 순으로 제한적 도입이다. 이벤트는 모든 사례가 0.85 이상이었고, affordance는 0.9 gate로 애매한 `훔쳐본다`·언어적 위협을 기존 규칙에 남길 수 있다. 챌린지는 매우 정확했지만 0.9 gate 커버리지가 70.83%라 기존 룰 게이트를 유지하고 회색지대만 Jev→nano 폴백으로 적용하는 편이 맞다.

제한: 실제 운영 로그가 아닌 수작업 표본이고, 이벤트 정답은 제작자 판단이 개입한다. nano 비교 프롬프트도 운영 `ChallengeClassifierService`의 모든 부가 필드를 재현한 것이 아니므로, 이 결과는 운영 전환이 아니라 canary 근거다.
