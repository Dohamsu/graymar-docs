...
**과금 모델 확정 (2026-07-23):** 채팅 종량제(1턴 5p)는 **정식 과금 모델**이다 — 초기 시장조사 잠정안 "정상 작동은 무료"(arch/76 D5 원칙 1)는 **폐기**됐다(CLAUDE.md 과금 원칙 참조). 5p 비율은 운영 상황에 따라 소유자가 조정한다(`POINTS_PER_CHAT` env — 코드 변경 불필요).
...
- **잔액 표시**: 데스크톱 Header `💎 N`(골드 옆) + **모바일 MobileStatusRow `💎 N`**(골드 옆, 탭=충전) + **모바일 햄버거 메뉴 "충전 (NP)" 항목**(2026-07-23). PointsModal은 레이아웃 공통 영역 마운트. pointsEnabled=false면 숨김.
- **모바일 상태줄 맥락 전환**(2026-07-23): 슬롯이 좁아 phase 기반으로 정보 스왑 — **전투(COMBAT)=HP·STA·골드·💎**(생존), **비전투=골드·💎·치안(Heat: Shield+안전/경계/위험+수치)·시간대**(탐험). 비전투도 부상 시(hp<maxHp) ❤️HP 경고 노출. 골드/💎 공통 요소 추출(배타 분기·중복 없음). phase=game-store.
...
### 서버 (구현·라이브 검증·배포 완료 2026-07-23 — server 1db5ce4)
| # | Task | 규모 | 상태 | 비고 |
|---|------|------|------|------|
...
### 클라이언트 (구현·배포 완료 2026-07-23 — client d97182e, Vercel)
| # | Task | 규모 | 상태 | 비고 |
|---|------|------|------|------|
...
코드 발급은 **admin API `/v1/admin/codes`** 한 곳. 서버 재시작 불필요(코드는 DB 즉시 저장).

### 발급 (토큰 하드코딩 없이)
```bash
curl -s -X POST http://localhost:3000/v1/admin/codes \
  -H "x-admin-token: $(grep '^ADMIN_TOKEN=' /Users/dohamsu/Workspace/graymar/server/.env | cut -d= -f2)" \
  -H 'Content-type: application/json' \
  -d '{"points":500,"maxRedemptions":10}'
```
응답 `"code"`(예: `3YYV-JWPE`)를 유저에게 전달 → 게임 내 💎 클릭 → 충전 모달 입력.

### 파라미터
| 필드 | 필수 | 설명 |
|---|---|---|
| `points` | ✅ | 지급 포인트. **5p=1채팅** → 500p=100채팅≈300원 (1p=0.6원) |
| `maxRedemptions` | 기본 1 | 몇 명이 쓸 수 있는지(다회용 공용). 유저당 1회는 자동(`unique(codeId,userId)`) |
| `expiresAt` | 선택 | ISO 만료일(예 `"2026-08-31T23:59:59Z"`). 생략=무기한 |
| `code` | 선택 | 커스텀 코드(예 `"BETA-2026"`). 생략=자동 `XXXX-XXXX`(혼동문자 0/O/1/I 제외) |

### 예시
```bash
# 1인 전용 (250p=50채팅)
-d '{"points":250,"maxRedemptions":1}'
# 커스텀+만료 (베타 이벤트, 50명)
-d '{"points":300,"maxRedemptions":50,"code":"BETA-2026","expiresAt":"2026-08-31T23:59:59Z"}'
# 발급 코드 전체 조회 (usedCount 확인)
curl -s http://localhost:3000/v1/admin/codes -H "x-admin-token: $(grep '^ADMIN_TOKEN=' server/.env | cut -d= -f2)"
```

### 발급 정책 · 주의
- **비용 상한 = 발급 총 포인트.** 예산만큼만 발급하면 그 이상 비용이 새지 않는다(파밍 불가 — 포인트는 코드로만 유입).
...
과금 모델은 채팅 5p 종량제로 확정(§2). 소유자 결정(2026-07-23) 반영.

- **[P1] 결제 연동 (자동 충전) — PG = 토스페이먼츠 (결제위젯) 확정, 구현은 후순위.** 현재는 어드민 수동 코드 발급뿐. 실런칭 시 토스페이먼츠 결제위젯으로 단건 충전(코드/포인트 구매) 연동 + 전자상거래법(사업자등록·청약철회·환불 정책). 단건 충전이라 정기결제 심사보다 가볍고, 현 코드→충전 구조와 호환. 수수료·정산·심사 요건은 구현 착수 시 공식 문서로 확인.
- **[P1] 어뷰징 방어 — 현행 유지(소프트 베타).** 이메일 인증 부재로 다계정 가입보너스(50p=10채팅) 파밍이 이론상 가능하나, **가입 보너스 50p는 현행 유지**(무료 체험 우선). 소프트 베타(지인·소수) 규모에서 실피해 미미로 판단, 수용. 공개 런칭 시 재검토 — 옵션 ① 이메일 인증 게이트(무료체험 유지) ② `SIGNUP_BONUS_POINTS=0` 쿠폰 전용(파밍 완전 차단·무료체험 소멸).
- **[P2] 운영 어드민 대시보드** — 포인트 사용/충전/코드 소진·유저별 소비·LLM 비용 추적을 볼 수 있는 어드민 화면(소유자 지시 2026-07-23, 기록만). 현재는 `point_transactions` 원장 + `llm_call_logs` 직접 쿼리로만 확인 가능.
- **[P2] 인프라 이전** — 서버·DB가 개인 맥 launchd + 로컬 Docker Postgres + cloudflared 터널. 단일 장애점 → 클라우드(서버 + managed Postgres) 이전이 실런칭 하드 블로커.
