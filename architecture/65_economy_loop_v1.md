...
## 부록 D — 엔딩 턴 서술 피날레 (2026-07-11, server 55da57f + 26baffb)

완주 2런 실측: RUN_ENDED 확정 턴의 서술이 일반 행동 묘사로 잘려 서술 차원의
마무리 부재 (엔딩 연출은 EndingScreen만 담당).

- **피날레 디렉티브** (prompt-builder): ui.endingResult 존재 시 [마지막 장면] —
  프롤로그 3막 지시와 대칭. 결과 매듭 + 새 인물·단서·질문 금지 + endingType별
  종결 톤(NATURAL/DEADLINE/PLAYER_CHOICE/DEFEAT) + 여운 문단.
  실측: "길었던 밤의 소음이 잦아들고, 코끝을 스치는 차가운 공기만이 지나온
  시간의 흔적을 일깨운다." — 장면이 처음으로 닫힘.
- **nano 선택지 스킵**: 엔딩 확정 턴은 클라가 엔딩 화면 전환 — 호출 1회 절약.
- **소개 연출 비활성** (26baffb): 엔딩 턴 소개는 다음 턴이 없어 실패 연출이
  마지막 인상으로 고정("내 이름은 투박한 노동자라고 하오" 실측) + 피날레
  지시와 충돌 — newlyIntroducedNpcIds 비움 + IntroFallback 삽입 금지.
