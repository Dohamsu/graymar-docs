...
## 9. V9 센서 정밀화 — 구현 (2026-07-26)

### 조사 중 추가 발견 2건 (§8 처방에 없던 것)

1. **`_npc_alias_pool` 이 `content/graymar_v1/npcs.json` 경로 하드코딩** — 별빛모래·카른홀트
   런이 **남의 팩 NPC 별칭 풀**로 판정되고 있었다. V9-c(무명 대사 의심)와 D5-2
   (CONTENT_ALIAS 분류)까지 함께 오염. §8이 지목한 "COMMON 팩 편향"보다 심각한 형태.
2. **`events_v2.json` 도 같은 하드코딩** — V10(이벤트-서술 NPC 정합) **게이트**가
   비-graymar 런에서 남의 팩 이벤트로 판정하고 있었다.

부수: 리포트 `meta` 에 실행 팩이 없어 사후 분석에서 팩을 알 수 없었다 → `meta.scenario` 추가.

### 구현

| 항목 | 내용 |
|------|------|
| 정본 분리 | `scripts/repetition_core.py` 신설 — 순수 로직(stem·감지·게이트)을 스크립트에서 분리. 인라인이면 **게이트 실패 경로를 실런 없이 검증할 수 없다**(스크립트가 top-level 실행). 서버 `narrative-filter.core` 패턴과 동일 |
| ① dedupe | 윈도우별 적재 → `{stem: 최대 카운트}` 로 흡수 |
| ② stem | `[가-힣]{2,8}` + 조사 분리(`으로`를 `로`보다 먼저) |
| ③ 대명사 | `PRONOUN_OPENERS`/`PRONOUN_STEMS` 를 정본으로 올려 V9·D5 공유 (구: D5 지역 정의) |
| ④ 팩 파생 | `extract_place_words(locations.json)` — 구 graymar 지명 8개 하드코딩 대체 |
| 별칭 분리 | CONTENT_ALIAS 는 게이트 제외 + 계측 라인에 별도 표기 |
| 게이트 | `evaluate_gate()` 불린 1개로 확정. 구 `len([i for i in v9_issues if "반복" in i]) <= 2` 는 **이슈 문면 substring 매칭**이라 계측 항목이 게이트로 샐 수 있었다 |
| 팩 인지 | `_PACK_ID`(= `--scenario`) 기반 `_CONTENT_DIR` — npcs·locations·events_v2 전부 |

### 임계 (77런 실측 보정)

어휘 반복은 arch/78·82가 추적 중인 **만성 문제**다 — 45%의 런에 6회+ 반복이 존재한다.
게이트는 만성 baseline을 재보고하는 곳이 아니라 **악화를 잡는 곳**이므로 임계를 baseline
위에 둔다. 전량은 계측 라인으로 항상 보이므로 추이 관찰은 임계와 무관하다.

- **심각도** `SEVERE_COUNT = 8` — 한 단어가 3턴 내 8회+ (상위 9%). 턴 수 편향 없음.
- **건수** `MAX_DISTINCT = 5` — 서로 다른 단어 6종+ (상위 4%). 광범위한 밋밋함 포착.
...
신 게이트 실패 5건은 **전부 심각도 경로**이며 전부 진짜 문체 반복이다 —
`훈련장×9`, `찻잔×8`(T8 한 턴에 "찻잔 가장자리" 3회), `장부×8·숫자×8`, `서류×8`.
대명사·조사 어절·팩 편향 아티팩트는 0건.

**§7에서 "아티팩트"로 판단하고 넘긴 arch/92 최종 런 2건은 신 센서에서 PASS 한다.**
`201759`의 `이름×10` 은 별빛모래 NPC 별칭 `이름 수집가`·`이름 잃은 귀환자` 였고,
팩 인지 수정 후 CONTENT_ALIAS 로 분류되어 게이트에서 빠진다 — 당시 판단이 데이터로 확인됐다.

### 검증

- `scripts/repetition_core_test.py` — 의존성 없는 유닛 **21케이스 전체 통과**.
  4대 결함 각각 + **게이트 양쪽 실패 경로**(심각도/건수) + baseline 통과 + 경계(짧은 런).
  구 구현 회귀("심각 1건이 건수 비교를 통과해 샌다")를 명시 케이스로 고정.
...
`장부×8`(장부 조사 장면)·`찻잔×8` 처럼 **장면 주제어**가 걸리는 경우가 있다. 주제어와
게으른 반복을 기계적으로 구분할 방법은 없고, 3턴에 8회면 주제어라도 변주가 필요한
수준이라 판단해 그대로 잡는다. 계측 라인이 단어·횟수를 그대로 보여주므로 사람이 즉시
판별할 수 있다 (구 센서는 `'장부의'×5`가 `'그가'`·`'아직'`과 섞여 나와 판별 불가였다).
