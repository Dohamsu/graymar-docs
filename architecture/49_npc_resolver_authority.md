...
## 부록 — `[NPC 등장]` 헤더 충돌 (2026-08-12, server 4cf3111)

NPC 결정 권한을 NpcResolver 로 단일화했음에도, **프롬프트 조립 단계에서 우회
경로**가 하나 남아 있었다. 같은 `[NPC 등장]` 헤더를 두 경로가 썼다:

| | 출처 | 성격 |
|---|---|---|
| ① | `directorHint.npcEntrance` (NanoDirector 연출 묘사) | 자유 텍스트. **어느 NPC 인지 모른 채** 작성됨 |
| ② | `ctx.npcInjection` (orchestration 결정) | "누가 등장하는지" 확정 지시 |

①은 NanoDirector fallback 경로(= NanoEventDirector 부재 턴) 전용이라 드물지만,
②와 겹치면 **서로 다른 인물을 등장시키라는 모순 지시**가 된다.

**실측 (1,353턴, 2026-07-25~)**
...
동시 25건 중 7건이 인물 수준 모순 — 예: ① "날카로운 눈빛의 회계사가 조용히
다가온다" vs ② "조용한 실무자가 이 장면에 나타납니다".

**조치**: ②가 있으면 ①을 주입하지 않는다 (`prompt-builder.service.ts`).
결정 권한이 NpcResolver 에 있으므로 자유 묘사가 양보하는 것이 정합적이다.
②가 없는 턴(① 단독 74턴)은 그대로 둔다 — 익명 배경 인물 등장은 시스템 프롬프트
`## 배경 인물`(신원 풀에 노점상·행상 등 명시)이 허용하는 정상 동작이다.

대안으로 검토한 "nano 프롬프트에 새 인물 창작 금지 지시 추가"는 택하지 않았다.
같은 프롬프트의 기존 부정 지시(`"그림자에서 나타남" "다가옴" 금지`)가 이미
무시되고 있고(동시 출현 25건 중 8건이 '그림자'), CLAUDE.md LLM 설계 원칙 4 가
지목하는 실패 양식이다. 서버 게이트가 옳은 자리.

### 검증의 한계 (기록)

- **증명된 것**: 유닛 4케이스(결정론) · 실런 30턴에서 `[NPC 등장]` 2개인 턴 0건 ·
  골든 스냅샷 무변화.
- **증명 못 한 것**: 개선 폭. 30턴에서 게이트가 0회 발동했다 (① 4턴 · ② 11턴이
  나왔으나 겹치지 않음). 충돌률 1.8% 에서 최소 1건 관측 확률은 30턴 43%,
  60턴 67% — **짧은 플레이테스트로는 측정 자체가 성립하지 않는다.**
- 조사·테스트런 어느 단계에서도 **측정 가능한 품질 피해는 확인되지 않았다**
  (주입 NPC 발화율: 충돌 4/25 vs 비충돌 58/590, 유의차 없음). 이 수정의 값은
  품질 개선이 아니라 **결정 권한 정합성**이다.
- 계측용 `[NpcEntranceGate]` debug 로그를 남겼다 — 실사용에서 발동 횟수가
  누적되면 그때 실제 빈도를 알 수 있다.
