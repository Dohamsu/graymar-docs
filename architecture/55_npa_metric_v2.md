...
## 부록 A — HAOCHE "-소" 종결 누락 수정 (2026-07-07)

### 발견 경위

CORE 5종 NPA 감사에서 로넨 어미 일치율 45%가 보고됨. 원문 대조 결과 로넨은
하오체(HAOCHE 등록)를 완벽히 지키고 있었고, 원인은 메트릭 자체였다:

- `REGISTER_PATTERNS.HAOCHE`에 하오체 **최빈 종결 "-소"**(있소/겠소/했소/놓았소)가
  누락 — `소이다`만 있고 `[가-힣]소`가 없었음.
- 말줄임(…)으로 분할된 "그저", "사실", "사람들이라 함은" 류 **말끝 흐림 파편**이
  분모에 포함 — speechStyle "말끝을 흐린다"인 NPC의 정상 발화가 위반으로 집계.

이로 인해 하오체를 지키는 NPC 전원이 45~59%로 오측정되어 TONE_DRIFT WARNING이
상시 발화했다 (진짜 이탈과 구분 불가).

### 수정 (scripts/e2e/audit/dialogue-quality.ts)

1. HAOCHE에 `[가-힣]소` 추가, HAPSYO에 `[가-힣]니다`(됩니다/갑니다), HAEYO에
   `[가-힣]요`(가요/돼요) 보강.
2. `isTrailingFragment()` — 3자 이하 조각·조사로 끝나는 미완결 구는 어체 판정
   불가로 분모 제외.

### 재채점 결과 (2026-07-07 CORE 5종 리포트, 동일 데이터)

| NPC | 어미 일치 (전→후) | 연결성 | 종합 |
|---|---|---|---|
| 로넨 | 45% → **100%** | 3.46 → 4.01 | 4.03 → 4.14 |
| 에드릭 | 50% → 88% | 3.59 → 3.97 | 3.25 → 3.33 |
| 하를런 | 59% → 94% | 3.60 → 3.95 | 4.16 → 4.22 |
| 마이렐 | 59% → 91% | 3.63 → 3.95 | 4.28 → 4.35 |
| 쥐왕 | 73% → 95% | 3.34 → 3.56 | 4.03 → 4.08 |

수정 이후 리포트의 어미 일치율은 이전 리포트와 직접 비교 불가 (기준 자체가 다름).
잔존 미스(에드릭 88% 등)가 이제 진짜 이탈 후보다. 서버 런타임의 R5 후처리
(llm-worker, 치환 기반)는 이 버그와 무관하게 정상.

---

## 부록 B — toneMatch 표본 붕괴 + HEAVY_WORDS 오탐 수정 (2026-07-08)

### 발견 경위 (에드릭 톤 2.50 연속 재현 분석)

에드릭이 두 런 연속 톤 일치도 2.50으로 "구조적 최하위"처럼 보였으나, 계측 데이터
분석 결과 세 가지 측정 결함의 합성이었다:

1. **표본 붕괴** — `classifyNpcTone`이 80~200자 발화를 전부 unknown으로 버려,
   10턴 감사에서 톤 축 평가 표본이 0~4턴 (발화가 긴 에드릭·평균 135자는 0턴).
2. **측정 불능의 점수화** — 평가 턴 0이면 matchRate 기본값 0.5 → 2.50점으로
   표시되어 "나쁨"과 구분 불가.
3. **HEAVY_WORDS substring 오탐** — '독'이 word boundary 없이 매칭되어
   "**유독** 소란스러운"이 poison '독'으로 집계. 유일한 실측 mismatch(T5)도
   오탐 제거 시 match로 반전 (조심 1건뿐 → cold baseline 기준 미달).

### 수정 (dialogue-quality.ts + types.ts + reporter.ts)

1. `classifyNpcTone` — heavy 신호 없으면 길이 무관 casual (unknown 대폭 축소).
2. `insufficientSample` 플래그 — 평가 턴 < 3이면 overall을 4축 평균으로 전환,
   리포트에 "측정 제외 — 표본 부족" 표기.
3. `countHeavyWords()` 단일 정본 — 무해 합성어(유독/고독/독특/감독 등) 제거 후
   카운트. classifyNpcTone과 baseline 보정의 중복 목록 통합.

### 재채점 결과 (2026-07-07 08시 리포트, 동일 데이터)

| NPC | 톤 (전→후) | 표본 (전→후) | 종합 |
|---|---|---|---|
| 에드릭 | 2.50 → **5.00** (mismatch 0) | 0 → 9턴 | 3.28 → 3.78 |
| 하를런 | 2.50 → 4.50 (mismatch 1 — 정당 감지) | 2 → 10턴 | 3.91 → 4.31 |
| 마이렐/로넨/쥐왕 | 5.00 유지 | 1~4 → 10턴 | 변동 없음 |

하를런 잔존 mismatch 1건은 "독이 든 성배 + 입 조심" (heavy 2건 정당 감지) —
수정 후 남는 신호가 진짜 신호임을 확인. 에드릭 "톤 문제"는 해소 (측정 결함이었음).
어미 메트릭(부록 A)과 동일하게, 수정 전후 톤 점수는 직접 비교 불가.
