...
- **2026-07-18 P5a~P5c (Phase 5 클라이언트)** (client f80da72..cabd0ac): **P5a** StartScreen 헬퍼 6종 분리(start-screen/ — PortraitLoadingOverlay/PresetCard/RadarChart/AuthForm+EmailInput/CreationLayout+StepIndicator/stat-config, 651줄; nextBonusStats는 기존 경로 re-export) — **2,481 → 1,842줄 (-26%)**, 게이트 smoke(클라 렌더)+regression(critical 0, AUTH 폼 실렌더). **P5b** game-store 헬퍼 918줄 분리(game-store.helpers.ts — 상태 매핑+내러티브 파이프라인, GameState type-only import로 런타임 순환 없음, **공개 훅 useGameStore 시그니처 완전 유지**) — **2,134 → 1,228줄 (-42%)**, smoke PASS. **P5c** StoryBlock 분리(narrative-text.tsx 렌더 유틸 정본 375줄 + SceneImageButton + NpcPortraitCard; **StreamTyper/TypewriterText는 once-guard·멱등성 제약으로 잔존**) — **1,291 → 709줄 (-45%)**. 게이트: /browse 실플레이 — 로그인→이어하기→자유 입력 턴 제출, **타이핑 진행 중 캡처("…갓 구운 빵의 고"에서 잘림) → 완성 문단 유지 확인(텍스트 소실 없음)**, 마커 누출 0·콘솔 에러 0. 발견: SceneImageButton은 원본부터 호출 0인 죽은 컴포넌트(이미지 생성 비활성 이력) — 분리 파일로 보존. smoke의 LLM latency 16.8s 경고 1회는 OpenRouter 순간 지연(클라 무관).
