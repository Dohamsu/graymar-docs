...
- 상태: ✅ main 병합(1d35c1c) · v2 + 로어북 게이트(9910fe7) · **2026-09-03 부터 Solar 전턴 운영(개발 기간, 교차 중단)**
...
## 6. 병합·v2·전턴 전환 (2026-09-03, 소유자 지시)

- **병합**: `feat/solar-dialogue-profile` → main 1d35c1c (브랜치 삭제, 정책대로). 
- **운영 전환**: `OPENAI_MODEL=upstage/solar-pro4`(메인), `LLM_ALTERNATE_MODEL` 주석 처리(교차 중단), allowlist `upstage/solar-pro4=Upstage`.
  프로파일은 슬러그로 자동 적용(HUB·LOCATION 전 턴 프롬프트에 접두·접미 확인). 원복 = `server/.env` 의 두 줄 되돌리기 + 재기동
  (Gemma `google/gemma-4-31b-it`, Luna 교차). 개발 기간 한정이며 비용은 표준가 기준 턴당 ≈2.9원(현 프로모 0.28).
- **§5.3 로어북 게이트**(9910fe7): 미발견 퀘스트 fact 는 로어북 블록에 주제 이름만 — 기록 없는 공개 채널 차단. 모델 무관.
- **v2**: 프로파일 접두에 대명사 개시 금지·호칭 단일·화폐/현대 사물 3줄 + 한 줄 다중 인용 분리(`split-multi-quote.core`, 라벨 상속)
  + 분류기 싱크대→개수대. 문법 파손은 계측만.
- 검증: 전턴 Solar 15턴 런 → §6.1.

### 6.1 전턴 Solar 검증 런 (2026-09-03, run 44fe6a0b graymar·HERBALIST·chatty 15턴, 서버 9910fe7)

| 지표 | 값 | 비고 |
|---|---:|---|
| 게이트 | 14/15 | V9 반복: '종이' 3턴 내 8회(T7) — Solar 어휘 재사용(종이·손끝·탁자·천막). penalty 0.4/0.3 적용 중인데도 |
| 원문 라벨 준수 | **97.0%** | v2 접두 + 분리 규칙. 저장본 무라벨 0·무명 0·라틴 0 |
| 어체 위반 | **10/32 (31%)** | 교차 런 8.1% 에서 급등. 유형: 하오체 NPC 의 `~겠지`(3)·`~요`·`보이소`(비문)·`구만`, 해체 NPC(레닉)의 `~구려`. R5v2 HAOCHE 교정 맵에 `겠지→겠소`·`구만→구려`가 없고 HAECHE 에 `구려` 역방향 교정이 없다 |
| 대명사 개시 | 7.1% (0.91/턴) | v2 접두 후에도 잔존 — 프롬프트 지시로는 안 내려간다(arch/78 와 같은 결론). 서버 후처리 필요 |
| 지연 narrative p50 | 3,835ms | 단일 모델 연속 호출 |
| 비용 | 프로모 0.28 (표준가 ≈2.9) | |
| 로그 | PromptProfile 28 · NpcMismatch:Kill 1(조용한 실무자 끼어들기 삭제) | MultiQuoteSplit 발동 0 (이 런에선 다중 인용 없음) |

**해석**: 라벨 축은 닫혔다(97%). 전턴 운영에서 새로 드러난 것은 **어체 드리프트** — 교차 런에서는 Gemma 홀수 턴이 대화 이력에 정상 어체를
남겨 앵커 노릇을 했는데, Solar 만 이어지면 `~겠지`·`~구만` 같은 반말·하게체 어미로 미끄러진다. 이건 프롬프트보다 **R5v2 교정 맵 확장**
(하오체: 겠지→겠소·구만→구려·~요→~오 잔존형 / 해체: 구려→구먼)이 맞는 처방이다(불변식 50 — 사후 교정은 어미 단위 결정론).
대명사 개시어도 같은 이유로 서버 후처리(문장 첫 대명사 → 별칭 치환 또는 삭제) 후보.

**다음(v3)**: ① R5v2 HAOCHE/HAECHE 맵 확장 + 스펙 ② 대명사 개시어 후처리(프로파일 모델 한정, 잠금 화자 별칭으로 치환) ③ 반복 어휘는
D5 계측으로 1주 관측 후 판단. 전턴 운영은 유지하되 어체 위반율을 `llm_speech_audit` 로 매일 본다(교차 대비 8.1% 가 기준선).
