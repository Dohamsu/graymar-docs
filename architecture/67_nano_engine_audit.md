...
### 잔여 ②③ 수정 완료 (server d5da578 + 53a50c9)

- **② 플레이어 자기 정보 기억**: rawInput 자기 공개 패턴(나는~/처음/일거리)
  감지 → `runState.playerDisclosures`(최근 5건) → 프롬프트 [플레이어가 이미
  밝힌 자기 정보] + 모순 금지 지시 (명시적 주입, nano 호출 없음).
  실측: 축적 확인 + NPC가 "뜨내기"로 정합 지칭 (모순 재발 없음).
- **③ 금전 증여 차감**: 대화계 ACTION + NPC 존재 + 증여 표현 감지 시
  specifiedGold ?? GIFT_DEFAULT_GOLD(2G, config) 차감·잔액 클램프·기존
  [골드] 이벤트 합류. 실측 "한잔 사 드시게" 미감지 → 패턴 보강 후
  [Gift] -2G 차감·서술 정합 확인. 회귀 12건.
