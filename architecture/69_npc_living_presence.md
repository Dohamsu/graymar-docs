...
> 상태: **B0~B4 구현·검증 완료** (2026-07-13). 후속(어미 다양화) 재배정 완료.
> 어체 검증 경로: **완결** — C1·C2·C2.5·C2.6(시스템 프롬프트 레거시 = 진짜
> 원인) 완료, HAPSYO 하오체 침식 80→9.1%로 **C3 미발동 확정** (게이트 미달).
...
> 개정 4: 2026-07-13 C2.5 확장판 — "나머지 이탈율" 분석(위반 3분류)에 따라
> C3 전 생성·계측 정비: 메인 forbidHint 하오체 종결 보강 + FOREIGN 하오체·
> 합쇼 계열 보완 + 낮춤체(해체↔반말) 인접 혼용 상호 허용. C3 게이트는
> C2.5 재측정 후 재판정으로 변경.
> 개정 5 (최종): 2026-07-13 C2.6 시스템 프롬프트 레거시 정정(진짜 원인 —
> "경어체로만" 전역 강제가 NPC별 register와 모순) + 검증기 5차 정밀화
> (ㅂ니다 활용·짧은 문장 필터) → 하오체 침식 80→9.1%, **C3 미발동 확정**.
> 어체 검증 경로 완결. §7 잔여 과제 통합 신설.
...
- **어체 준수 계측** (✅ C2 구축 완료 — `turns.llm_speech_audit`): 화자별 대사
  추출 × validateSpeechRegister, C1~C2.6 전 과정의 판정 근거로 사용됨. NPA
  어미 메트릭(arch/55) 교차 검증은 미실행 (§7 잔여 과제).
...
**Phase C2.5 — 생성·계측 정비 확장판 (2026-07-13 구현 완료, 재측정 대기)**

배경 — "C2.5+C3로 나머지 이탈율(HAECHE 25%·BANMAL 29%)도 잡히는가"에 대한
분석. 위반은 성질이 세 갈래이고, 각각 처방이 다르다:

| 위반 유형 | 해당 | 처방 |
|-----------|------|------|
| ① 격식 침식 (하오체가 타 어체 침범) | HAPSYO 52% 본체 + HAECHE 일부 | 생성 억제(forbidHint) + 계측 보완 + 잔여만 C3 |
| ② 인접 혼용 (해체↔반말) | HAECHE 25%의 주성분 | **위반이 아님** — 계측 재정의로 제외 (재생성 교정은 개성 훼손) |
| ③ 표본 부족·오검출 | BANMAL 29%(표본 7) | 재측정으로 정화 |

구현 3건 (+유닛 10케이스, `dialogue-generator.validate-register.spec`):

1. **메인 프롬프트 forbidHint 하오체 종결 보강** (speech-register.ts):
   HAPSYO `'~이오/~해요/~야'` → `'~이오/~하오/~소·~겠소·~시오/~해요/~야'`,
   HAECHE·BANMAL에도 `~소·~겠소` 명시. **핵심 발견**: dialogue_slot 경로
   (dialogue-generator REGISTER_RULES.forbidden)엔 `~소/~오/~하오`가 이미
   있었고, **메인 LLM 프롬프트 정본에만 빠져 있었다** — C2 실측 위반(로넨
   "지나치겠소" 등)이 전부 메인 경로에서 난 이유.
2. **FOREIGN_ENDINGS 보완** (validateSpeechRegister): 낮춤체(HAECHE/BANMAL)의
   혼용 감지가 `이오|하오`만 봐서 문장 중간 "~했소/~겠소" 침식을 놓치던 것
   (HAECHE 하오체 침식 과소 집계 원인) + `합니다|입니다`만이라 "있습니다"를
   놓치던 것을 HAOCHE 기준과 동일하게 보완. 유닛이 이 누락을 실증 후 수정.
3. **낮춤체 인접 혼용 상호 허용**: HAECHE↔BANMAL은 어미가 크게 겹치고
   (지/거든/어/잖아) 혼용이 자연스러워, 종결 판정을 두 어미 집합의 합집합으로
   통합. "해체 NPC의 반말 어미"를 위반으로 세던 계측 노이즈 제거 — 개성
   어긋남은 speechStyle 톤 가이드의 몫으로 분리.

**예상 효과와 재측정 계획**: ①로 HAPSYO 생성 단계 이탈이 줄고(단 forbidHint는
soft 지시라 0은 아님 — 원칙 4), ②③으로 HAECHE/BANMAL 수치가 "진짜 침식만
남게" 정화된다. playtest 재측정(`llm_speech_audit`) 후 **C3 발동 게이트를
재판정** — C2.5만으로 HAPSYO가 임계(15%) 아래면 C3(재생성, UX 이질감 상시
부작용)는 하지 않는 게 최선.

**재측정 1차 시도 무효 (2026-07-13)**: C2.5 빌드 재기동 후 playtest 40턴을
실행했으나 **OpenRouter API 키 총한도 초과(403 Key limit exceeded)**로 40턴
전원 LLM FAILED — `llm_speech_audit` 0건, 측정 무효 (run 7a0eb64f, 연속
playtest 세션 누적으로 한도 소진). 코드 원인 아님 — 유닛 10케이스로 검증기
동작은 확정.

**재측정 결과 (2026-07-13, 한도 복구 후 40턴 — run 10d93c63)**:

| register | 대사 | 위반 | 위반율 | 판정 |
|----------|------|------|--------|------|
| HAOCHE | 27 | 0 | **0%** | ✅ 안정 유지 (이전 3.9%) |
| HAPSYO | 5 | 4 | **80%** | ⚠️ **진짜 이탈 지속** — 전 샘플 하오체 종결 |
| HAECHE | 2 | 1 | (50%) | 유일 위반이 "~건가" 어미 누락 **오검출** → 수정 |

- **HAPSYO 위반 4건 전부 실이탈** (오검출 0): 펠릭스 "오셨소?", 로넨
  "오셨구려"·"잘 되었소…있소"·"조심하시오…있소". **forbidHint 보강(soft
  지시)만으로는 하오체 관성을 못 누른다는 것이 실증** — 원칙 4(soft 지시
  무시)의 재확인. → **C3 HAPSYO 단독 파일럿 발동 조건 충족.**
- HAECHE 오검출("그냥 구경하는 건가?")은 낮춤체 의문 어미(건가/인가/는가/
  을까) 3차 정밀화로 즉시 수정 (유닛 11케이스).
- 표본 주의: HAPSYO 5·HAECHE 2로 작다. 단 HAPSYO는 방향이 명확(4/4 실이탈)
  해 게이트 판정에 충분. **향후 계측은 사용자 지시로 10~15턴 다회 누적**
  방식으로 전환 (40턴 단회 금지 — 키 한도·시간 비용).

**Phase C2.6 — 시스템 프롬프트 하오체 강제 레거시 정정 (2026-07-13 — 진짜 원인)**

C2.5 재측정에서 HAPSYO 이탈이 지속(4/5)되자 원인을 역추적 — **시스템
프롬프트 두 곳에 전원-하오체 시절 레거시가 남아 있었다** (C1의 P3 치환과
동류, 프롬프트 층):

- 메인 58행: "NPC 대사는 경어체(~이오/~하오/~소)로만" ← 64행의 "NPC별
  어체를 따르라"와 **정면 모순**. ⚠️ 강조가 붙은 전역 강제라 NPC 블록의
  forbidHint(약한 지시)가 눌림 — C2.5가 안 먹힌 이유.
- 66행: '"너"는 금지' ← 반말·해체 playerRef("너/자네")와 충돌.
- 파티 프롬프트(~300행): 동일 하오체 강제 + "경어체 기본".

**정정**: "그 NPC에 명시된 어체(speechRegister)로만 — 합쇼체 NPC가 하오체로
말하면 캐릭터가 깨집니다" + 합쇼체 금지를 서술 문장으로 스코프 한정 + 지칭을
어체별 playerRef 인지형으로. 파티 프롬프트는 "명시 어체 우선, 미명시 시
경어체 기본". 로넨이 새던 건 관성이 아니라 **모순된 지시 중 강한 쪽을 따른
것** — 입력 단계 해결(원칙 6)이 정답이었다.

**재측정 (15턴 × 2세션 누적 — 이후 계측은 10~15턴 다회 누적 규칙)**:

| register | 대사 | 실위반 | 하오체 침식(게이트 대상) |
|----------|------|--------|--------------------------|
| HAPSYO | 11 | 2 (18.2%) | **1건 (9.1%)** — 펠릭스 하오체 |
| HAOCHE | 27 | 2 (7.4%) | — (존댓말화 1·감탄 혼잣말 1) |
| HAECHE | 1 | 1 | 1건 (레닉 "비켜주시오", 표본 1 — C2.5 FOREIGN이 정확 감지) |

- **HAPSYO 하오체 침식 80% → 9.1%** — C2.6이 진짜 원인이었음을 실증.
- 재측정 검수가 **검증기 오검출 2종을 추가 발견 → 5차 정밀화** (유닛 +2):
  ① ㅂ니다/ㅂ니까 불규칙 활용 누락("모릅니다/아닙니다/겁니까"가 위반 처리)
  ② 문장 길이 필터(>3자)가 "…있소"/"가시오" 같은 짧은 정상 종결문을 탈락시켜
  마지막 문장 판정이 밀리던 버그. 저장 샘플 10건 오프라인 재판정으로 교정
  수치 확정 (NOW-PASS 5건).

**C3 게이트 최종 판정: 미발동 확정** — HAPSYO 하오체 침식 9.1% < 임계 15%.
"C2 위반율이 낮으면 C3를 아예 하지 않는 게 최선"(§UX 이질감) 원칙대로 C3
(재생성)는 착수하지 않는다. `llm_speech_audit`는 상시 계측으로 유지 — 향후
10~15턴 누적 측정에서 임계 재초과 시에만 C3를 재검토한다.
...
- **C3 착수 조건 재확인**: HAPSYO만이라도 실이익이 크다(합쇼체 NPC 절반이
  ...
  **HAPSYO 단독 파일럿 → NPA·이질감 실측 → 확대 여부 판단**의 단계로 좁게
  시작한다.
- **(C2.5 이후 갱신)**: 위 수치는 C2.5 이전 베이스라인이다. C2.5(생성 억제 +
  계측 정화)가 들어갔으므로 **재측정 수치로 게이트를 다시 판정**한다 —
  HAPSYO가 임계 아래로 내려오면 C3는 발동하지 않는다.
...
| **C2.5** 생성·계측 정비 | ✅ 완료 | forbidHint 보강 + FOREIGN 보완 + 낮춤체 상호 허용 + 의문 어미. 재측정에서 HAPSYO 지속(4/5) → 원인이 상류에 있음을 시사. |
| **C2.6** 시스템 프롬프트 레거시 정정 | ✅ 완료 — **진짜 원인** | "NPC 대사는 ~이오/~하오/~소로만" 전역 강제(메인+파티)가 NPC별 register와 모순. 정정 후 **HAPSYO 하오체 침식 80→9.1%**. 검증기 5차 정밀화(ㅂ니다 활용·짧은 문장 필터, 유닛 18). |
| **C3** 선별 재생성 | ✅ **미발동 확정** | 게이트(15%) 미달 — 9.1%. 재생성의 UX 이질감을 치르지 않는 최선 경로. llm_speech_audit 상시 계측 유지, 임계 재초과 시에만 재검토. |
...
**테스트 인프라 자산**: `turns.llm_speech_audit`(C2 저장)는 상시 계측원이다 —
B0의 `llm_npc_reaction`과 함께 "품질 회귀를 수치로 감시"하는 런타임 계측
2종을 확보. C3가 미발동으로 닫혔으므로 이 컬럼의 역할은 **회귀 감시**다
(임계 재초과 시에만 C3 재검토).

---

## 7. 잔여 과제 통합 (2026-07-13 정리 — 본문 4곳 산재분 취합)

본문 각 절에 흩어져 있던 백로그를 한 곳에 취합한다. **전부 "다음 사이클"
항목**이며, 위 완결 판정(B0~B4·C1~C2.6)의 미완을 뜻하지 않는다.

### 마감 항목 (완결 선언에 필요)

| 항목 | 내용 |
|------|------|
| **커밋** | ✅ 완료 (2026-07-13). 어체 작업 전체(C1~C2.6 + 검증기 정밀화 + 유닛 18)를 server `576d6b9`·docs `47e25fa`로 커밋·푸시, 서버 재시작으로 실행 반영(`/v1/version` 일치). |
| **사용자 수동 테스트** | 2026-07-13 사용자 직접 플레이 검증 예정 — 결과에 따라 후속. |

### 검증 보강 (여유 시)

| 항목 | 출처 |
|------|------|
| NPA 어미 메트릭 교차검증 — 오프라인 audit ↔ 런타임 `llm_speech_audit` 위반율 일치 1회 대조 | C2 |
| 파티 던전 경로 실측 — C2.6이 파티 프롬프트도 정정했으나 솔로 경로만 실측함 | C2.6 |
| 표본 보강 — C3 미발동 근거가 HAPSYO 11대사(방향 명확·통계 얇음), HAECHE/BANMAL 진짜 이탈율 미확정. 10~15턴 다회 누적으로 자연 축적 | C2.6 판정 |

### 기능 백로그 (다음 사이클)

| 항목 | 출처 |
|------|------|
| 장소 재방문 변화 언급 ("지난번보다 경비가 늘었군") — locationDynamicStates | B4-3 |
| npcRelations 동적 변화 (사건이 관계 톤을 바꿈) — 런타임 오버레이 설계 필요 | B4-3 |
| silverdeen_v1 npcRelations 콘텐츠 보강 | B4-3 |
| rel: 쿨다운 완전 지속 — turns.service 기록 경로 (기존 recentTopics 기록 자체가 불완전, 반복 실측 시) | B4-1 정정 |
| 근황 대상 마커 후처리 방어 (부재 인물 마커 무효) — 프롬프트로 안 잡힐 때 | B4 오귀속 |
| factsParts 총량 예산 가드 — enforceTotal이 memoryParts만 커버 (불변식 17 사각) | B4-3 |
| BG NPC 어미 반영 강화 (경량 프롬프트) | 어미 다양화 |
| NPC 간 말투 유사도 계측기 (NpcDistinctness는 개별 준수만 측정) | 어미 다양화 |
| dialogue_slot/메인 서술 어체 검증 이원화의 근본 수렴 — 별도 설계 문서 | C 장기 |
