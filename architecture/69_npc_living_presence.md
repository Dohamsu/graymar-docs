...
**정밀 베이스라인 (2026-07-13, 어미 2차 정밀화 후)**:
- 2차 정밀화: HAPSYO 의문형(습니까/십니까/ㅂ니까/인가/는가) + HAECHE 확장
  (걸세/라네/더라/더라고/그래/게/라니까) 어미 추가. 오검출 대폭 감소.

| register | 대사 | 위반율 | 판정 |
|----------|------|--------|------|
| HAOCHE | 36 | 5.6% | 안정 (세계관 기본) |
| HAECHE | 2 | 0% | 안정 |
| **HAPSYO** | 7 | **71.4%** | **진짜 이탈** |

- **HAPSYO 71%는 오검출이 아니라 실제 이탈**이다. 위반 샘플이 거의 전부
  하오체 종결: 로넨 "지나치겠소?", 메신저 "오셨소?", 펠릭스 "서시오!/밝히시오!",
  브렌 "묻고 싶소/아니오". **합쇼체 NPC들이 하오체 관성으로 대거 새어난다** —
  세계관 하오체 기본값이 합쇼체를 침식(해요체 이탈과 같은 뿌리). HAOCHE(5.6%)·
  HAECHE(0%)는 안정.
- **C3 게이트 판정 (정밀)**: **C3 대상은 HAPSYO에 집중**한다(HAOCHE/HAECHE는
  안정하므로 개입 불필요). 합쇼체는 세계관에 적합한 어체(군·귀족 격식)이나 LLM이
  하오체로 회귀하므로, C3 선별 재생성으로 어미를 강제하는 게 정당하다. 표본
  확대(추가 세션 누적)로 HAPSYO 이탈률을 재확인 중.
- 잔여 오검출 의심 1건(브렌 "주십시오…아닙니다" 정상 합쇼인데 위반 표기) — C3
  구현 시 재생성 게이트를 "하오체 종결 명확 감지" 기준으로 좁혀 오검출 대사
  재생성을 차단할 여지.
