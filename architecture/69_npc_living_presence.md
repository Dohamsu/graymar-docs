...
**구현 완료 + 베이스라인 (2026-07-13)**:
- 배선: `validateSpeechRegister` export + `auditUtteranceRegisterCore`(순수,
  유닛 3) + turns.`llm_speech_audit` JSONB 컬럼 + done 후처리 계측(등장
  NPC×register×extractNpcUtterances, 무명·미배정 스킵). 텍스트 미변경. 전체
  1105 passed.
- **계측이 validateSpeechRegister 버그 3건을 발견**(C2가 제대로 작동한 증거) —
  ① HAOCHE "구려/구료" 어미 누락 ② HAPSYO FOREIGN이 "십시오"의 "시오"·"오"를
  하오체로 오검출(`[소오]`→`[가-힣]소` + `(?<!십)시오`) ③ HAECHE "네/구먼/
  게나" 어미 누락. dialogue_slot 경로도 쓰는 검증기라 **기존 버그** 동시 수정.
  수정 후 HAOCHE 26.9%→5.3%.

| register | 대사 | 위반율 | 해석 |
|----------|------|--------|------|
| HAOCHE | 19 | 5.3% | 세계관 기본 — 안정 |
| BANMAL | 1 | 0% | 표본 부족 |
| HAECHE | 23 | 26.1% | 반말 혼용 이탈(레닉 등) + 잔여 오검출 |
| HAPSYO | 3 | 66.7% | 하오체로 새어남(로넨 "말이오/되었소") — 표본 작음 |

- **핵심**: HAOCHE만 안정(5%)이고 **합쇼체·해체 NPC가 하오체/반말로 이탈**
  (로넨·레닉·브렌 실증). C1으로 텍스트 오염은 멎었으나, **생성 단계 어체
  이탈은 남아 있다** — 재배정 어미가 LLM 하오체 관성에 눌리는 §어미 다양화
  진단과 일치.
- **잔여 계측 노이즈**: "걸세/라네/그래/인가" 등 세밀한 해체·의문 어미가
  아직 오검출된다(어미 사전이 방대해 완벽 불가). C2 목적(대략 위반율로 C3
  판단)엔 충분하나, HAPSYO/HAECHE 절대 수치는 상한(오검출 포함)으로 읽는다.
- NPA 교차검증(arch/55)은 오프라인 audit 실행 필요 — 별도.

**C3 게이트 판정 (1차)**: HAPSYO·HAECHE 위반율이 임계(15%)를 초과하나, ① 표본이
작고(HAPSYO 대사 3) ② 잔여 오검출이 수치를 부풀린다. **C3 착수 전 표본 확대
재측정 + 잔여 어미 오검출 추가 정밀화로 "진짜 위반율"을 먼저 확정**해야 한다
(성급한 C3 재생성은 오검출 대사까지 건드릴 위험). 즉 C2는 "이탈이 실재하고
계측 가능"까지 확립했고, C3 발동은 정밀 베이스라인 확정 후로 유보한다.

**정밀 베이스라인 (2026-07-13, 어미 2차 정밀화 + 3세션 누적)**:
- 2차 정밀화: HAPSYO 의문형(습니까/십니까/ㅂ니까/인가/는가) + HAECHE 확장
  (걸세/라네/더라/더라고/그래/게/라니까) 어미 추가. 오검출 대폭 감소.
- 표본 확대: playtest 40턴 × 3세션 누적 (register별 표본 확보).

| register | 대사 | 위반 | 위반율 | 판정 |
|----------|------|------|--------|------|
| HAOCHE | 76 | 3 | **3.9%** | ✅ 안정 (세계관 기본) |
| HAECHE | 12 | 3 | 25.0% | ⚠️ 이탈 (반말 혼용) |
| BANMAL | 7 | 2 | 28.6% | ⚠️ 이탈 (표본 작음) |
| **HAPSYO** | 21 | 11 | **52.4%** | ⚠️ **진짜 이탈 (최다)** |

- **HAPSYO 52%는 실제 이탈**(오검출 아님). 위반 샘플이 거의 전부 하오체 종결:
  로넨 "지나치겠소?", 메신저 "오셨소?", 펠릭스 "서시오!/밝히시오!", 브렌 "묻고
  싶소/아니오". NPC별로는 **로넨 8중 5·브렌 7중 3**에 집중 (문시 2/0은 준수).
  **합쇼체 NPC가 하오체 관성으로 대거 새어난다** — 세계관 하오체 기본값이
  합쇼체를 침식(해요체 이탈과 같은 뿌리).
- **HAOCHE 3.9%로 매우 안정**(76 대사). 세계관 기본 어체라 LLM이 자연 준수.
  HAECHE(25%)·BANMAL(29%)도 임계 초과하나 표본이 작다(12·7).
- 잔여 오검출 의심(브렌 "주십시오…아닙니다" 정상 합쇼인데 위반 표기) — C3
  게이트를 "하오체 종결 명확 감지" 기준으로 좁혀 오검출 대사 재생성을 차단한다.
...
**C3 발동 대상 확정 (정밀 베이스라인 3세션 반영, 2026-07-13)**:
- **1순위 HAPSYO (52%, 대사 21)** — 표본 충분·이탈 명확(로넨·브렌 하오체 종결).
  C3 착수 시 여기부터. 게이트: 합쇼체 NPC 대사가 **하오체 종결(~소/~오/~시오/
  ~겠소, "십시오" 제외)로 명확히 끝날 때만** 재생성 후보 (오검출 브렌 "주십시오…
  아닙니다" 류 차단).
- **보류 HAECHE(25%, 12)·BANMAL(29%, 7)** — 임계 초과이나 표본이 작아 확정
  전 표본 추가 확보 필요. HAECHE는 "반말 혼용"이 실이탈인지 캐릭터 개성(레닉
  술꾼 횡설수설)인지 구분이 모호 — 재생성이 개성을 훼손할 위험.
- **제외 HAOCHE (3.9%)** — 안정. 개입 시 순손실.
- **C3 착수 조건 재확인**: HAPSYO만이라도 실이익이 크다(합쇼체 NPC 절반이
  하오체로 새는 것 교정). 단 §UX 이질감(보강 3)이 상시 부작용이므로, C3는
  **HAPSYO 단독 파일럿 → NPA·이질감 실측 → 확대 여부 판단**의 단계로 좁게
  시작한다.

---

## 어체 검증 경로 (C1~C3) 종합 현황 (2026-07-13)

| 단계 | 상태 | 요지 |
|------|------|------|
| **C1** 레거시 무해화 | ✅ 완료 | 하오체 강제 치환(자네→그대) 제거. 해체 NPC "그대" 오염 0(실측). |
| **C2** 화자 인지 계측 | ✅ 완료 | validateSpeechRegister export + auditUtteranceRegisterCore(유닛 3) + turns.llm_speech_audit 저장. 계측이 검증기 버그 **5건**(구려·십시오·[소오]·네/구먼/게나·의문형) 발견·수정 — dialogue_slot 경로도 개선. |
| **C3** 선별 재생성 | ⏸ 착수 대기 | 정밀 베이스라인 확정. **HAPSYO(52%) 단독 파일럿**부터, 하오체 종결 명확 감지 게이트 + done 교체(R7) + 과변형 가드. HAECHE/BANMAL 보류, HAOCHE 제외. |

**개발 설계 요약**: C3는 done 최종본에서 `auditUtteranceRegisterCore`가
반환한 HAPSYO 위반 대사 중 **하오체 종결이 명확한 것만** `extractNpcUtterances`
로 잘라, dialogue-generator 재생성 인프라(LLM_DIALOGUE_MODEL, "의미 유지·어미만
교정" positive)로 재생성 → 의미 유사도 가드 통과 시 done 교체(R7 프로토콜),
실패 시 원문 유지. 텍스트를 regex로 깎지 않고 재생성으로 대체(문장 파괴 회피).

**테스트 인프라 자산**: `turns.llm_speech_audit`(C2 저장)는 C3 전후 A/B의
상시 계측원이다 — B0의 `llm_npc_reaction`과 함께 "품질 회귀를 수치로 감시"하는
런타임 계측 2종을 확보. C3 파일럿의 성공 판정도 이 컬럼의 HAPSYO 위반율
하락으로 측정한다.
