...
재배정을 적극 훼손**하는 것을 발견. 수정은 미착수(방향 결정 대기).
...
- **A안 (최소·즉효)**: P3 `자네→그대`·`이보게→듣고 계시오` 전역 치환 제거.
  해체 NPC "자네"는 정상. 하오체 NPC가 드물게 "자네" 섞는 건 프롬프트 어체
  가이드(`buildRegisterLines`)가 1차 방어.
- **B안 (근본·큰 작업)**: 서술을 @마커별로 분리해 **화자 어체 기준으로**
  검증·교정 — dialogue_slot 수준 fallback을 메인 대사에도 이식.

#### 남은 부작용·고려사항

- **A안 부작용**: 하오체/합쇼 NPC가 LLM 관성으로 "자네"(해체 호칭)를 섞을 때
  자동 교정이 사라진다. 프롬프트 가이드 준수율에 의존 — 재배정 후 하오체가
  10명으로 줄어 노출 빈도는 감소했으나, 오사용 시 방어막이 얇아짐.
- **B안 복잡도**: 후처리 위치(1626행)가 **서술 전체 텍스트**를 다루므로 화자
  어체를 모른다. 마커별 분리 교정은 (a) @마커↔화자 npcId↔speechRegister 매핑
  (b) 대사 구간만 정확히 잘라내기 (c) 교정이 문장 구조를 깨지 않게 — 3중
  난이도. 어미 교정 자체가 문장 파괴 위험(CLAUDE.md LLM 원칙: 사후 삭제 최후
  수단)이라, 교정보다 재시도(재생성)가 안전하나 메인 서술은 스트리밍이라
  재생성 비용이 큼.
- **이원화 자체가 근본 취약**: dialogue_slot(검증 O)과 메인 서술(검증 X)이
  갈려 있어, 같은 대사라도 어느 경로로 생성됐느냐에 따라 어체 보장이 다르다.
  장기적으로는 대사 생성을 dialogue_slot 경로로 수렴시키는 게 정공법이나,
  스트리밍 UX(2-Phase 렌더)와 충돌해 별도 설계 필요.
- **P2 로그 노이즈**: SPEECH_VIOLATION이 이제 정상 다중 어체를 대량 오검출해
  `[NarrativeFilter]` 로그가 과다해진다. A안과 함께 정규식을 화자 어체 인지형
  으로 바꾸거나 최소한 하오체 전제 부분을 제거해야 로그 신뢰도 유지.
- **SPEECH_VIOLATION 검출만 하고 미교정**이라, 현재도 "위반 감지→방치" 상태다.
  즉 메인 대사는 사실상 어체 무검증이며, 유일하게 텍스트를 바꾸는 P3만 (잘못된
  방향으로) 작동 중.
