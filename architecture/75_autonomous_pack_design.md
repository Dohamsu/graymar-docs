...
> 상태: 📐 P1~P4 구현 + **G2 관문 통과** (2026-07-16) — [[74_autonomous_narrative_direction]] 문답 14건 확정 정본. **P0 스파이크 통과 §12 · G2 프로토타입 §13 · P4 구현 로그 §15 · G2 정식 계측 통과 §16**(15턴×3 실런: 결함 2건[생성 폴백·채택 스티키니스] 발견·수정 후 채택 8·규명 7·품질 10/10). **이중 관문(P0·G2) 충족 → P5~P8 대공사 커밋 가능.** 코드는 server `feat/dynamic-npc-registry`(P0~P4+G2수정, main 미병합), AUTONOMOUS 팩 없어 프로덕션 dormant.
...
## 16. G2 정식 계측 결과 — ✅ 통과 (2026-07-16)

정식 G2를 graymar 임시 AUTONOMOUS 픽스처(narrativeMode+motifs 10개, port 3100 격리)로 **15턴×3** 실측. §14.2가 "최대 미검증"으로 지목한 "플레이하며 시드를 공정하게 풀어내는가"를 검증. **두 설계-미작동 결함을 계측으로 발견·수정 후 통과.**

### 16.1 발견·수정한 결함 2건 (server `783d400`)

- **결함1 — plotSeed 항상 폴백**: `maxTokens 1000` + nano(light 모델)로 진상 JSON(truth+casting+keyFacts 8+…)이 **잘려 파싱 실패→폴백 템플릿**("단서 1"). 스모크에서 `generatedByFallback:True` 실측. **수정**: maxTokens 2500 + **메인 모델(Gemma 4 26B)** + 타임아웃 30s(§13 프로토타입이 메인 모델 기준이었음 — 구현이 nano로 이탈). → 4/4 정합 생성.
- **결함2 — 비트 채택 0**: 대화 스티키니스로 채택 0 실측(§15.4 예고의 극단). `beatAvail=true`인 4턴이 전부 CONVERSATION_CONT(조사·관찰도 SOCIAL이라 대화 연속으로 빠짐). **수정(§15.4 반영)**: `determineTurnMode` **규칙 1.5 beat 우선 창** — A(탐색 행동 INVESTIGATE/OBSERVE/SEARCH: 세계 상호작용) + C(`BEAT_FORCE_AFTER_TURNS=4` 강제창: 정체 방지). NPC 명시 지목은 여전히 최우선(Player-First 보존). 단위 4케이스.

### 16.2 수정 후 3런 실측

| 지표 | 수정 전 | 수정 후(3런) |
|------|---------|-------------|
| plotSeed 정합 생성 | 폴백만 | **3/3 (fallback 0)** |
| 비트 선계산 턴 | 33 | 29 |
| **비트 채택** | **0** | **8** |
| **keyFact 규명** | **0** | **7** (fact 7종, 표류 경미 FACT_4만 2회) |
| **동적 NPC 자연 발생** | 미확인 | **2** (§15.4 잔여 검증됨) |
| turnMode 균형 | CONV 편중 | WORLD 16 / CONV 13 / PLAYER 1 |
| 품질(playtest V) | — | **10/10 × 3** |

### 16.3 판정

- **§11-B "정합·공정·해결가능한 미스터리 생성" ✅** — 메인 모델 수정 후 진상 품질 우수("에드릭 장부 조작→자금 세탁→실종 조작", keyFacts 논리 연결). 독립 규명 경로 실증.
- **§5.1 "플레이하며 시드를 풀어내는가(채택 루프)" ✅** — 채택 8·규명 7로 자율 루프 실작동. 대화 스티키니스는 규칙 1.5로 균형점 확보(대화 완전 차단 아님).
- **AUTHORED 무회귀 ✅** — 1244 passed, 픽스처 원복 후 세 팩 AUTHORED 확인.
- **다음 = P5(종결 파이프)** — 규명율(7/9~10)·acts 소진·게이지 임계를 엔딩으로 잇는다. G2 통과로 P5~P8 대공사 커밋 가능(§8 이중 관문 충족).

**잔여(P8 이월)**: 규명율 분포의 회차 편차, premise n-gram 다양성 정밀 계측은 다회(10~15턴 다수) 축적 몫. 현 3런은 "루프가 작동한다"까지 확증.

## 17. P5 종결 파이프 구현 (2026-07-16)

G2 통과 후 §6 종결을 구현. AUTHORED(Incident resolved)와 별개 종결 축.

- **순수 모듈 `autonomous-ending.ts`** (16테스트): `computeClearanceRate`(규명율=발견/전체 keyFacts) · `checkAutonomousEnding`(① acts.turnBudget 총합 소진 ② packMeter endingTrigger 임계 — 둘 다 최소 15턴 불변식19) · `selectEndingTone`(규명율 band HIGH≥0.7/MID/LOW<0.3 × 게이지 종결 시 gaugeCollapse 오버레이).
- **팩 계약 `ScenarioMeta.endingTones`** — 규명율×게이지 톤 매트릭스. 미선언 시 팩 중립 fallback(NEUTRAL_TONE).
- **`turns.service` 배선** — checkEndingConditions 뒤 AUTONOMOUS 오버라이드(규명율 계산·게이지 임계 판정), generateEnding 후 endingResult에 규명율 톤 오버레이(endingType/closingLine/clearanceRate/band). **truth 불변** — 엔딩은 "얼마나 규명했나"만 반영, 진상은 안 바꾼다. generateEnding endingReason에 AUTONOMOUS_ACTS/GAUGE 추가.
...
## 19. P8 1차 계측 캠페인 (2026-07-17)

카른홀트 12턴 × 3런 + 그레이마르 12턴 × 2런 (arch/76 D4·D1-c와 통합 실행 — playtest `directionMetrics` 정본 지표 사용).

### 19.1 결과 요약

| 지표 | 카른홀트 (자율) | 그레이마르 (저작) |
|---|---|---|
| V-체크 | 8/10 × 3 (V1·V2 실패 = 자율 팩 특성: 저작 incident 부재) | **10/10 × 2** |
| 3-gram 반복률 / 인접 자카드 | 0.006 / 0.002 | 0.010 / 0.004 |
| distinct 이벤트 비율 | 1.00 | 1.00 |
| 이벤트 소스 | PROC 6·FREE_CONV 12·FREE_PLAYER 4·**BEAT 2** | EVT 7·FREE_CONV 8·FREE_PLAYER 1 |
| 비트 채택 | **0 / 2 / 0** (런별) | — |
| 의도 정합 채택률 | 1/2 (누적 표본 2/5 = 40%) | — |
| 규명율 | 0/? · 2/9 · 0/10 | — |
| 스레드 억제 위반 | 4~6건/런 (봇 무작위 행동 영향 포함) | 5건/런 |
| D3 감정 변동 | 런당 2~3명 (suspicion 최대 46.5) | 런당 5~6명 |
| D3-c′ REPORT/APPROACH | 미발동 (임계 60 미달 — 무작위 봇 12턴 한계) | 미발동 |

### 19.2 발견 결함 — plotSeed 비동기 동결 lost update (P0, 수정 완료 `041cfba`)

kh_1 런에서 **생성 성공 로그에도 불구하고 최종 runState에 plotSeed 부재** → 12턴 전체가 디렉터 없이 진행 (채택 0의 실원인). `generatePlotSeedAsync`가 SELECT→전체 되쓰기(CAS 없음)라 seed 생성 ~10초 사이 커밋된 턴과 경합 — seed 유실 또는 (반대 방향) stale 스냅샷의 턴 상태 롤백 가능. **jsonb_set 부분 패치로 원자 갱신** 교정, 경합 재현 검증(턴 3커밋 병행 → seed 생존·위치·이력 보존). P6 핫픽스(async 동결)가 만든 회귀 — async화 시 arch/60 부분 패치 원칙을 놓친 사례.

### 19.3 판정·후속

- **반복 서사 방어**: n-gram·distinct·무진행 전부 건강 — "무한 생성·무진행" 신호 없음 (D4 baseline 확보).
- **디렉터 존재감이 낮다 (§15.4 재확인)**: seed 정상 런에서도 채택 0~2/12턴, BEAT 2 vs FREE_CONV 12. 원인 분해 — ① 대화 스티키니스(무작위 봇도 대화 위주) ② WORLD_EVENT 턴과 비트 신선도(1..3턴) 창 미스매치. **후속 판단 필요**: stale 창 확대 or WORLD_EVENT 승격 조건 완화 vs "대화 위주 플레이에 디렉터가 얌전한 것"을 의도된 균형(불변식 47)으로 수용. 소유자 결정 대기.
- **의도 정합 채택률 표본 5** (2/5) — 아직 판단 불가, 계속 축적.
- **D3-c′ REPORT/APPROACH 미발동** — 무작위 봇으로는 감정 임계 도달 불가 (suspicion 최대 46.5<60). 검증하려면 지향 시나리오(적대 연타) 필요 — FLEE는 기검증.
- 스레드 억제 위반 4~6건/런은 **봇 행동 산물 포함** — 실플레이어 대비 상한 추정치로만 사용.

### 19.4 디렉터 존재감 중간안 — TRAVEL 선계산 + 커밋 클로버 차단 (2026-07-17, server a35453c)

§19.3 후속 판단을 소유자 결정으로 **중간안** 채택: 신선도 창(2턴)·의도 존중 가드(D1)는 무변, 공백 지점만 메꾼다.

- **설계 정정 과정 (정직 기록)**: 최초 안("도착 턴에 직전 비트 유효 처리")은 성립 불가 — 모든 비트가 생성 당시 장소로 구속(`locationId` 고정)이라 도착 턴엔 장소 하드 차단에 걸린다. → **공급 측 해소**로 전환: HUB·이동 턴에도 선계산(TRAVEL 모드, `locationId` 없는 장소 무관 비트 — 소문·전갈·마주침류). 도착 턴(WORLD_EVENT 확정)에 age 1 비트가 존재하게 됨.
- **근본 결함 추가 발견 — 커밋 클로버**: 검증 런에서 채택이 여전히 0 → 원인은 **동기 턴 커밋의 runState 전체 되쓰기가 워커가 쓴 nextBeatCandidates를 낡은 스냅샷으로 매번 되돌리는 레이스** (beatAge 4→8 고착 실측 — 빠른 페이스에서 거의 매 턴). 워커 fresh CAS(arch/60)는 쓰기 측만 지켰고 **커밋 측이 뚫려 있었다**. plotSeed 유실(§19.2)의 잔여 경로도 동일 — 041cfba는 쓰기 원자화만이라 불충분했다. **수정**: AUTONOMOUS 커밋(commitTurnRecord)에 한해 `jsonb_set` 병합으로 plotSeed·nextBeatCandidates는 DB 현재값 보존(채택 소비 턴만 payload null 정본). AUTHORED 경로 무변.
- **검증**: 동일 조건 12턴 봇 런 — 채택 **0 → 2**(BEAT_11_1 OBSERVE 정합 채택 "폐갱 은맥"·BEAT_12_2), beatAge 신선(2턴대) 유지, keyFact 2 규명, 폐기 0. 1341 passed.
- **잔여 관찰**: 도착 턴 자체는 이번 런에서 PROC(직전 HUB 턴 워커 완료 타이밍) — 채택은 도착 직후 턴들에서 발생. 워커 소유 중첩 필드(propsTraces·감정 shift 등)는 여전히 클로버 가능(soft data 수용) — 전면 병합은 과공학으로 보류, 문서화만.
