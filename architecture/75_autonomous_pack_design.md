...
| 단계 | 내용 | 상태 | 커밋 |
|------|------|------|------|
| P4-1 | BeatCandidate 타입 + RunState(nextBeatCandidates·plotProgress) + 킬스위치 config | ⬜ | — |
| P4-2 | PlotDirectorService(비트 후보 2~3 nano 생성) + 인력 순수 함수 + 유닛 | ⬜ | — |
| P4-3 | 워커 배선 — AUTONOMOUS 런 서술 후 비트 선계산 → applyRunStatePatch 저장 | ⬜ | — |
| P4-4 | 턴 동기 채택 — matchBeatCandidate(순수) + eventDef 주입 + 후보 소비 + 폴백 보존 | ⬜ | — |
| P4-5 | keyFact 발견 추적(plotProgress.discoveredKeyFactIds) + AUTONOMOUS fact 공개 경로 | ⬜ | — |
| P4-6 | dev 픽스처(graymar 임시 AUTONOMOUS) + 라이브 E2E 멀티턴 검증 | ⬜ | — |
