...
### D5 — 과금 3원칙 등재 〔P0 · 문서만〕 ⚠️ 원칙 1 폐기 (2026-07-23)
CLAUDE.md 설계 불변식 인근에 등재 (지금 코드 변경 없음, 미래 결정 봉인):
1. ~~**정상 작동은 무료**~~ — **폐기 (2026-07-23)**. 초기 테스트 단계의 잠정 결론이었고, 실런칭 비용 구조 판단으로 **채팅 1턴 = 5p 과금**을 정식 채택(arch/85). 아래 §3 "정상 작동에 과금 = 최악 평가 축" 경고도 이 결정으로 재평가됨 — 무한 무료가 아니라 소액 종량제(1턴 5p)로, 실패 턴 무과금·투명 잔액으로 조사 우려를 완화한다.
2. **과금은 부가가치만** → 채팅 종량제로 대체·부가가치는 보조. (이미지·문체 프리셋·추가 캠페인/팩은 유효.)
3. **실패 턴 무과금** — 오류·빈 응답·재생성에 비용 차감 금지 (retry-llm 무료 유지). — **유지** (arch/85 환불 2경로).
