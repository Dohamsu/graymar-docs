...
## 부록 M — NPC 선제 단서 억제: 대화 계열 fact 게이트 (2026-07-13)

목표(사용자): NPC가 살아있는 존재처럼 다채롭게 활동하고, 이방인이 말을
걸었을 때 별도 대사가 없는 한 먼저 단서를 언급하거나 부자연스럽게 행동하지
않게 한다. 본 부록은 A축(선제 단서 억제) — B축(NPC 살아있음: memory/agenda/
schedule 대화 반영)은 후속 트랙.

### 진단

fact 공개 트리거 `INFO_ACTIONS`에 **TALK 포함** → 그냥 "말을 건다"도
SUCCESS/PARTIAL이면 fact 공개 경로. `selectRevealableFact`는 주제 키워드
매칭이 없어도 **순서 fallback 공개**(arch/60 확률 게이트로 완화했으나 여전히
확률적으로 샘). 그래서 평범한 잡담·안부 TALK에 NPC가 묻지 않은 단서를
흘림. 시스템 프롬프트의 "선제 정보 전달 금지" 규칙(F·line 162)은 soft라,
fact가 questReveal로 주입되면 LLM이 무시하고 대사에 녹임(CLAUDE.md LLM
원칙 4: soft 지시 무시 / 입력 단계 해결 우선).

### 수정 (입력 단계 차단)

fact 비주제 fallback 게이트에 **행동 계열 분기** 추가:
- **대화 계열**(TALK/PERSUADE/TRADE/HELP): 주제 매칭(matchedByTopic) 없으면
  fact 공개 **완전 차단** — 플레이어가 그 화제를 명시적으로 물어야 공개.
- **조사·탐색**(INVESTIGATE/SEARCH/OBSERVE): 능동 탐색이라 기존 확률
  fallback 유지.
- **BRIBE/THREATEN**: bypassTrust 면제(대가를 치른 정보 요구).
- 차단된 fact는 기존대로 뇌물 기회(bribeOpportunity)로 이월 — 정보 흐름
  자체는 보존, "선제 노출"만 차단.

### 검증 (실측)

잡담 TALK 3건("근처 사람에게 말을 건다", "요즘 동네 어떤지", "날씨 이야기")
→ questReveal **0건**(단서 차단). 명시 질문("사라진 장부에 대해 묻는다")
→ questReveal 공개. 서버 빌드 + 전체 1087 passed + 재시작.

### 후속 (B축 — 백로그)

단서를 막은 자리에 NPC 자기 상태를 채운다: NpcSchedule(일과)·agenda·
NpcPersonalMemory를 대화 톤에 주입, immediateGoal을 "정보 파악" 일변도에서
NPC 자기 목적(장사·경계·잡담)으로 다양화. "부자연스러움 제거(A)" 후
"다채로움 부여(B)" 순서.
