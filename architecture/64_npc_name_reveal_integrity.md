...
- R6: 미조우 NPC 실명의 콘텐츠 경유 유입(npcRelations·facts versions·lorebook)은
  안전망(npcStates 순회) 사각 — **현상 유지로 확정 (2026-07-10 사용자 결정)**.
  근거: 실측 0건(선별 주입이 미조우 텍스트 유입 자체를 억제) + 전 콘텐츠 치환의
  오치환/비용 리스크 + 제3자 언급은 서사적으로 정당한 공개 경로일 수 있음.
  실제 사례 관측 시 'heard-of(전해 들은 이름)' 상태 도입을 우선 검토.
