...
## 부록 B — 문서·코드 대조 감사 발견 2건 (2026-07-05)

부록 A의 #4·#6이 "완료"로 기록됐으나 실제 diff 대조 결과 각각 한 곳씩 미반영이 남아있었음. 수정 완료:

| # | 발견 | 조치 |
|---|------|------|
| 9 | #6(토크나이저 단일화)이 3곳 중 2곳만 반영 — `context-builder.service.ts`의 `collectFactKeywords`(fact 매칭용, 부록 A #6이 명시한 지점)는 원본 정규식 중복을 그대로 유지, `extractKoreanKeywords` import는 미사용 상태로만 추가돼 있었음 | `collectFactKeywords`를 `extractKoreanKeywords` 호출로 교체 |
| 10 | #4(공유 별칭 STRONG 가드)가 메인 매칭 블록(1a)에만 적용되고 `matchParticleAll`(조사 호명 매칭)은 고유성만 체크 — "현장에 있음" OR 분기 누락 (1a와 비대칭) | `matchParticleAll` 시그니처를 `ctx` 인자로 확장, `isAtLocation` OR 분기 추가로 1a와 동일 가드로 정렬 |

재검증: 관련 스펙 5개(`npc-resolver.partial-name`, `context-builder.focused-npc/direction-hint/deadline`, `quest-progression.select-fact`) 58개 테스트 통과, 변경 파일만 대상 lint 신규 이슈 0건, `pnpm build` 성공.

변경 파일: `llm/context-builder.service.ts`, `engine/hub/npc-resolver.service.ts`.
