...
### 0.4 완성도 A-1~A-4 + 3차 게이트 결함 수정 (2026-08-26, server 7eb59f2·4ff84c6)

**A-1** 여정 아카이브 autoBondLabel kind 우선(연인/벗/막역한 벗/원한/맞수) +
유형별 마무리 한 줄. **A-2** 관계 선물(arch/108) 지급 시 `FIRST_GIFT` 마일스톤
(`appendRelationMilestone`). **A-3** 동행 자동 파탄(BROKEN): 배신 GRUDGE/RIVAL
전이 시(워커 CAS) + 동행 NPC 도주 시 — companionNpcId 잔존으로 도주 만료 후
복귀하던 구멍 봉합. **A-4** RELATION 알림 토스트 — 마일스톤(직전 턴 단일 규칙,
이중 알림 방지) + 동행 성립/해산/파탄 (조사 정본 korParticle).

**3차 실런(T27~T32)이 잡은 결함 2건 (수정 4ff84c6):**

- **판정·서술 데스싱크** — 유르마 고백 T27: 서버는 수락(ROMANCE 기록)했는데
  서술은 거절 톤("농담을 하시는 것이겠지요"). 판정 결과가 서술에 주입되지
  않았다. 판정을 **프롬프트 빌드 전 1회**로 옮기고 같은 결과를 `[관계 전이]`
  디렉티브와 CAS 기록에 공용 (arch/58 기록·서술 단일화 원칙. CAS 재판정 폐기 —
  fresh ±5 오차보다 서술=상태 일치 우선).
- **HOSTILE_BREAK nano 사문** — 검으로 직접 공격해도 신호 NONE (nano 3차
  연속 무발동 — 고백과 동일 패턴). L1 폴백 추가: actionType FIGHT(parsedType
  정본) = 배신 신호. 코어 게이트(무유형+trust>-10 무변화)가 낯선 이 전투
  오탐을 자연 차단.

**재검증 (T31~T32) PASS**: 공격 → GRUDGE + BETRAYED + 동행 즉시 파탄 →
다음 턴 알림 2건("유르마 — 지난 배신"·"동행이 깨졌습니다") + 유르마 위치
스케줄 복귀(LOC_SS_RIB — 오버라이드 해제 증명).

**관찰(설계 판단 → §0.5 에서 결정)**: kind 는 NPC 별 독립이라 동시 복수
연인이 가능 (이렌·유르마 동시 ROMANCE 실측).

### 0.5 복수 연인 정책 — "금지하지 않되 들키면 대가" (2026-08-26 소유자 결정, fc031eb)

소유자 결정: 금지 대신 **현실 모델** — 마주치지 않고, 접점이 없고, 소문이
돌지 않으면 괜찮다. 들킴은 전부 서버 결정론 (`affair.core.ts`, 스펙 19):

- **① 직접 목격**: 애정 표현 턴(`detectAffectionExpressionL1` — 고백 문형의
  상위 집합)에 다른 연인이 같은 장소(`npcLocations` 정본). 동행 중인 연인은
  항상 같은 장소이므로 자연히 최고 위험 — 동행×양다리의 창발 성립.
- **② 제3자 목격 → 소문**: 같은 장소의 소개된 CORE/SUB → `affairRumors`
  적재 + 시그널 피드 RUMOR 1건 (TTL 24tick=2일, 대상별 중복 방지 — 플레이어는
  피드로 "소문이 돌기 시작했다"를 보고 위험을 인지할 수 있다).
- **③ 소문 도달**: 소문 생존 중 다른 연인과 대면(일반 턴 포함) → 폭로.
- **폭로 결과**: 기존 배신 경로 재사용(BETRAYED + GRUDGE — 티어 힌트 원한
  오버라이드 자동) + 감정 타격(trust −35·attach −15·susp +30) + 동행 즉시
  파탄 + `[관계 폭로]` 디렉티브(헤더 등재) + 알림(파탄 당턴·배신 익턴).
- **연인 1명이면 완전 무동작** — 애정 표현마다 소문·판정이 돌면 노이즈
  (불변식 54 시그널 자가오염 교훈).

### 0.6 B-2 관측 — 동행 중 스토리 진행 12턴 (2026-08-26, run c46a0b45 T26~T40)

유르마(연인) 동행 상태로 수녀원·검은얼음 시장 퀘스트 진행. **스토리 저해 없음**
(fact 3 발견·S0→S1 전환) — 관측된 개입 패턴 3종:

1. **타 NPC 대화 중 끼어들기 0건** (수녀원 4턴 — 루오르·미렌 대화·조사에서
   유르마 마커·언급 완전 0). 독점 없음 ✓. 다만 **존재감도 0** — 곁에 있는데
   그림자도 안 비침. "매 턴 대화를 주도할 필요는 없다" 지시가 전면 침묵으로
   작용. 완급(짧은 반응 허용) 조정은 관측 누적 후 판단.
2. **무화자 턴 화자 흡인** — 시장 조사(NpcResolver NO_NPC) 턴에 LLM 이
   동행자를 화자로 세움 → LockSeed(서술 화자 백필)가 actionHistory 에 기록
   → **다음 턴 대화 잠금이 유르마로 성립**. 장면 전환 직후·무대상 조사에서
   동행자가 자연 화자가 되는 것 자체는 서사적으로 자연 (실측 대사도 맥락 정합).
3. **잠금이 명시 지목을 이긴 1건은 콘텐츠 결함** — "꿈약 조제사에게" 지목이
   카일룬 텍스트 매칭 실패(roleKeywords 미저작 + role 자동 추출이 '조제사'
   미포착) → 동행자 잠금이 응답을 가로챔. 카일룬 roleKeywords 저작으로 해소
   (star_sand CORE/SUB 18명 전원 미저작 — 자동 추출 실측 동작해 카일룬만
   외과 수정, '귀환자⊃환자' 충돌 교훈으로 확대 보류).

**실런(T33~T45) PASS — 직접 목격 경로 전체 체인**: 마레크 친분 5턴(trust
5→78) → 고백 수락("연인은 받아들이겠네" — **[관계 전이] 디렉티브로 판정·서술
일치 확인**) → 동행 성립 → 여관 이동(마레크 추종) → 이렌 포옹·사랑 고백 →
**마레크 WITNESSED 폭로**: GRUDGE + BETRAYED, trust 96→61·susp 0→30, 동행
즉시 파탄("동행이 깨졌습니다: 마레크가 곁을 떠났습니다" — korParticle 정상),
다음 턴 "마레크 — 지난 배신" 알림 + 위치 갈비평원 복귀. 소문 경로(②③)는
코어 스펙으로 검증 (실런은 목격자 소개 상태 조성 비용 대비 보류).
